...
| ディレクトリ | 対象 | データセット |
|---|---|---|
| [`Organic/`](./Organic) | 有機分子 | QM9（標準12物性）+ NMR拡張2物性 |
| [`Metal_complex/`](./Metal_complex) | 遷移金属錯体 | tmQM（5物性）+ tmQMg NMR拡張1物性 |
| [`paper/`](./paper) | 参考論文 | — |

各物性フォルダは `scripts/`（グラフ構築・学習・評価）と `results/`（ベースライン vs PFP記述子付与の比較プロット）で構成しています。元データ・PFP記述子・学習済みグラフ/チェックポイントはサイズの都合上、本リポジトリには含めずローカルでのみ管理しています。
...
本プロジェクトの中核成果は以下のarXivプレプリントにまとめています（詳細: [`paper/`](./paper)）。
...
> [arXiv:2602.03046](https://arxiv.org/abs/2602.03046) (2026).

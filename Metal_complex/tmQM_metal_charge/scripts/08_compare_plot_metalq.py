...
"""
tmQM Metal_q予測 - Baseline vs PFP+BO 比較プロット
"""
...
class EGNN_PFP_BO_MetalQ(nn.Module):
    """EGNN with PFP and Bond Order for Metal_q"""
...
def main():
    ...
    BASELINE_MODEL_PATH = os.path.join(TMQM_DIR, "training_baseline_egnn_metalq2/best_baseline_model.pth")
    BASELINE_GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_baseline_metalq")
    
    PFP_BO_MODEL_PATH = os.path.join(TMQM_DIR, "training_pfp_bo_egnn_metalq/best_model_bo.pth")
    PFP_BO_GRAPH_DIR = os.path.join(TMQM_DIR, "qgraphs_tmQM_pfp_metalq_bo")
    
    OUTPUT_DIR = os.path.join(TMQM_DIR, "comparison_plots_metalq")
    ...
    # =============================================
    print("\n" + "="*70)
    print("📊 Baseline Model Evaluation (Metal_q)")
    print("="*70)
    ...
    # モデル読み込み
    baseline_model = EGNN_Baseline_MetalQ(input_dim=5, hidden_dim=128, num_layers=7).to(device)
    ...
    # =============================================
    print("\n" + "="*70)
    print("📊 PFP+BO Model Evaluation (Metal_q)")
    print("="*70)
    ...
    # モデル読み込み
    pfp_bo_model = EGNN_PFP_BO_MetalQ(input_dim=261, hidden_dim=128, 
    ...
    pfp_bo_preds, pfp_bo_targets, pfp_bo_mae, pfp_bo_r2 = evaluate_model(
        pfp_bo_model, pfp_bo_loader, device
    )
    
    print(f"✓ PFP+BO MAE: {pfp_bo_mae:.4f}")
    print(f"✓ PFP+BO R²:  {pfp_bo_r2:.4f}")
    
# =============================================
    # 比較プロット（横並び・統一サイズ版）- Metal_q修正版
    # =============================================
    print("\n" + "="*70)
    ...
    ax1.set_xlabel('True Metal Charge', fontsize=24, fontweight='bold')
    ax1.set_ylabel('Predicted Metal Charge', fontsize=24, fontweight='bold')
    ...
    ax2.set_xlabel('True Metal Charge', fontsize=24, fontweight='bold')
    ax2.set_ylabel('Predicted Metal Charge', fontsize=24, fontweight='bold')
    ...
    # 保存パスの定義
    plot_path = os.path.join(OUTPUT_DIR, 'baseline_vs_pfp_bo_metalq_comparison.png')
    ...
    print(f"PFP+BO MAE:    {pfp_bo_mae:.4f}  (R²: {pfp_bo_r2:.4f})")
    ...
    ax.set_xlabel('True ', fontsize=38, fontweight='bold')
    ax.set_ylabel('Predicted', fontsize=38, fontweight='bold')
    ...
    # 保存パスの定義
    overlay_plot_path = os.path.join(OUTPUT_DIR, 'baseline_vs_pfp_bo_meta_q_overlay.png')

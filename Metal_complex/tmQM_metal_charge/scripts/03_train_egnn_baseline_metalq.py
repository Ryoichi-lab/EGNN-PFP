...
"""
tmQM 金属錯体 Metal_q予測 - EGNN Baseline
EGNN原著論文準拠（PFPなし、原子タイプのみ）
"""
...
# =============================================
# EGNN Baseline モデル（Metal_q用）
# =============================================

class EGNN_Baseline_MetalQ(nn.Module):
    """
    EGNN Baseline - Metal_q予測用（PFPなし）
    ...
    def __init__(self, 
        ...
        # Metal_q予測ヘッド
        self.metalq_head = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.SiLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.SiLU(),
    ...
    def forward(self, batch):
        ...
        # グラフレベルの集約（mean pooling）
        graph_embedding = scatter(h, batch.batch, dim=0, reduce='mean')
        
        # Metal_q予測
        metalq_pred = self.metalq_head(graph_embedding)
...
def train_epoch(model, dataloader, optimizer, device):
    ...
    for batch in tqdm(dataloader, desc="Training", leave=False):
        ...
        outputs = model(batch)
        metalq_pred = outputs['metal_q'].view(-1)
        metalq_true = batch.y.view(-1)
        
        # MSE Loss（論文準拠）
        loss = nn.MSELoss()(metalq_pred, metalq_true)
        mae = torch.mean(torch.abs(metalq_pred - metalq_true))
...
def validate_epoch(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Validation", leave=False):
            ...
            all_predictions.append(metalq_pred.cpu().numpy())
            all_targets.append(metalq_true.cpu().numpy())
...
def plot_predictions(predictions, targets, epoch, mae, output_dir, split_name="Val"):
    ...
    textstr = f'EGNN Baseline (tmQM)\nEpoch: {epoch}\n{split_name} MAE: {mae:.4f}\nR²: {r2:.4f}'
    props = dict(boxstyle='round', facecolor='lightcoral', alpha=0.8)
    plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=14,
            verticalalignment='top', bbox=props)
    
    plt.xlabel('True Metal_q', fontsize=14, fontweight='bold')
    plt.ylabel('Predicted Metal_q', fontsize=14, fontweight='bold')
    plt.title(f'EGNN Baseline - {split_name} - Epoch {epoch}', fontsize=16, fontweight='bold')
    plt.legend(fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    plt.savefig(os.path.join(output_dir, f'baseline_predictions_{split_name.lower()}_epoch_{epoch}.png'), dpi=200)
    plt.close()

def main():
    # 絶対パス設定
    TMQM_DIR = "/home/users/uchiyama/tmQM_dipole"
    GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_baseline_metalq2")
    OUTPUT_DIR = os.path.join(TMQM_DIR, "training_baseline_egnn_metalq3")
    ...
    print("="*70)
    print("tmQM 金属錯体 Metal_q予測 - EGNN Baseline")
    print("EGNN原著論文準拠（PFPなし、原子タイプのみ）")
    print("="*70)
    ...
    print(f"  Metal_q: {sample.y.shape}")
    ...
    num_epochs = 1000  # tmQMは大規模なので500エポック
    ...
    print(f"\n{'='*70}")
    print("⚙️  EGNN原著論文準拠設定（Baseline）")
    print(f"{'='*70}")
    print(f"  Dataset:       tmQM (金属錯体)")
    print(f"  Target:        Metal_q (金属電荷)")
    ...
    print(f"  Layers:        7")
    print(f"  Input:         5-dim (atom_type1 + geom4)")
    print(f"  Edge feat:     1-dim (distance only)")
    print(f"  Coord update:  Enabled")
    print(f"  Pooling:       Mean")
    print(f"  Split:         Stratified by metal")
    print(f"  PFP:           Disabled (Baseline)")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training - tmQM Baseline (Metal_q, No PFP)")
    print(f"{'='*70}\n")
    ...
    for epoch in range(num_epochs):
        ...
        print(f"  Val:   MAE={val_mae:.4f}  |  LR={current_lr:.6f}")
    ...
    # ベストモデルをロード
    best_checkpoint = torch.load(os.path.join(OUTPUT_DIR, 'best_baseline_model.pth'))
    ...
    ax1.set_title('EGNN Baseline (tmQM Metal_q) - MAE', fontsize=14, fontweight='bold')
    ...
    ax4.set_xlabel('True Metal_q', fontsize=12, fontweight='bold')
    ax4.set_ylabel('Predicted Metal_q', fontsize=12, fontweight='bold')
    ax4.set_title(f'Test Results (MAE: {test_mae:.4f}, R²: {r2:.4f})', 
    ...
    stats = {
        'dataset': 'tmQM',
        'target': 'metal_q',
        'model_version': 'egnn_baseline_no_pfp',
        ...
        'model_config': {
            'input_features': '5-dim (atomic_num1 + geom4)',
            'edge_features': '1-dim (distance only)',
            'hidden_dim': 128,
            'num_layers': 7,
            'coord_update': True,
            'edge_inference': True,
    ...
    print(f"\n{'='*70}")
    print("🎉 EGNN Baseline Training Complete (tmQM Metal_q)!")
    print(f"{'='*70}")

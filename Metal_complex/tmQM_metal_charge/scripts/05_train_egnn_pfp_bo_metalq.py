...
"""
tmQM 金属錯体 Metal_q予測 - EGNN×PFP+BO
...
# =============================================
# EGNN×PFP+BO モデル（Metal_q用）
# =============================================

class EGNN_PFP_BO_MetalQ(nn.Module):
    """
    EGNN×PFP+BO - Metal_q予測用
...
def main():
    ...
    GRAPH_DIR =  "/home/users/uchiyama/tmQM_dipole/qgraphs_tmQM_pfp_metalq_bo2"
    OUTPUT_DIR = os.path.join(TMQM_DIR, "training_pfp_bo_egnn_metalq3")
    ...
    print("="*70)
    print("tmQM 金属錯体 Metal_q予測 - EGNN×PFP+BO")
    ...
    print(f"  ターゲット: {sample.y.shape} (Metal_q)")
    ...
    model = EGNN_PFP_BO_MetalQ(input_dim=261, hidden_dim=128, edge_dim=5, num_layers=7).to(device)
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training - EGNN×PFP+BO (Metal_q)")
    print(f"{'='*70}\n")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae:
            ...
            checkpoint = {
                ...
                'val_mae': val_mae,
                'history': history,
                'model_type': 'egnn_pfp_bo_tmqm_metalq'
            }
    ...
    ax1.set_title('EGNN×PFP+BO (Metal_q) - MAE', fontsize=14, fontweight='bold')

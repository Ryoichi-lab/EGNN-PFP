...
# XYZファイルとMetal_q参照値のパス
XYZ_DIR = "/home/users/uchiyama/tmQM_dipole/datasets/tmqm/tmqm/tmQM/xyz_files"
METAL_Q_REFERENCE_PATH = "/home/users/uchiyama/tmQM_dipole/datasets/tmqm/tmqm/tmQM/tmQM_y.csv"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(TMQM_BASE_DIR, "graphs_tmQM_baseline_metalq2")
OUTPUT_DIR_PFP = os.path.join(TMQM_BASE_DIR, "graphs_tmQM_pfp_metalq2")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("tmQMデータセット 金属錯体 Metal_q用グラフ作成")
print("EGNN-PFP 金属錯体拡張版")
print("="*60)
print("\n目的:")
print("  1. EGNNベースライン（PFPなし）")
print("  2. EGNN×PFP（PFP記述子あり）")
print("  の性能比較（金属錯体・Metal_q）")
...
def load_metalq_reference(reference_path):
    """
    tmQM_y.csvからMetal_qの参照値を読み込む
    
    フォーマット: CSD_code;Electronic_E;Dispersion_E;Dipole_M;Metal_q;HL_Gap;HOMO_Energy;LUMO_Energy;...
    ...
    try:
        ...
        # Metal_qカラムが存在するか確認
        if 'Metal_q' not in df.columns:
            print(f"⚠️ 警告: 'Metal_q'カラムが見つかりません")
            return {}
        ...
        for idx, row in df.iterrows():
            ...
            # NaNチェック
            if pd.isna(csd_code) or pd.isna(metal_q):
                continue
            
            # 両方の形式で登録
            metalq_dict[csd_code] = metal_q
            metalq_dict[f"{csd_code}.xyz"] = metal_q
        
        print(f"✓ {len(df)} 分子のMetal_q参照値を読み込みました")
        print(f"  Metal_q範囲: {df['Metal_q'].min():.4f} - {df['Metal_q'].max():.4f}")
        
        # サンプル表示（.xyz形式）
        sample_items = [(k, v) for k, v in list(metalq_dict.items())[:6] if k.endswith('.xyz')]
        print(f"  サンプル (.xyz形式): {sample_items[:3]}")
        
        return metalq_dict
        
    except Exception as e:
        print(f"⚠️ Metal_q参照値の読み込みエラー: {e}")
        import traceback
        traceback.print_exc()
        return {}
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, metalq_reference=None, cutoff=5.0, debug=False):
    """EGNNベースライン用：XYZファイルからグラフデータを作成（PFPなし、Metal_q用）"""
    try:
        atoms, positions = read_xyz_file(xyz_path)
        ...
        # Metal_qの取得（CSVからのみ）
        metal_q = None
        if metalq_reference and mol_id in metalq_reference:
            metal_q = metalq_reference[mol_id]
        
        if metal_q is None:
            if debug:
                print(f"  {mol_id}: Metal_qが取得できません（CSVに存在しない）")
            return None
...
# ===== PFP版：グラフ作成関数 =====
def create_graph_from_xyz_pfp(xyz_path, pfp_descriptors, mol_id, metalq_reference=None, cutoff=5.0, debug=False):
    """PFP版：XYZファイルとPFP記述子からグラフデータを作成（Metal_q用）"""
    try:
        atoms, positions = read_xyz_file(xyz_path)
...
# ===== グラフデータ作成関数（ベースライン用） =====
def create_molecular_graphs_baseline(mol_ids, xyz_dir, metalq_reference, split_name):
    """ベースライン用：グラフデータ作成（PFPなし、Metal_q用）"""
    graph_data_list = []
    failed_molecules = []
    ...
    for mol_id in tqdm(mol_ids):
        ...
        graph_data = create_graph_from_xyz_baseline(
            xyz_path=xyz_path,
            mol_id=mol_id,
            metalq_reference=metalq_reference
        )
...
# ===== グラフデータ作成関数（PFP版） =====
def create_molecular_graphs_pfp(mol_ids, descriptor_data, xyz_dir, metalq_reference, split_name):
    """PFP版：グラフデータ作成（Metal_q用）"""
    graph_data_list = []
    failed_molecules = []
...
# ===== データ分割（金属種による層化分割） =====
def stratified_split_by_metal(mol_ids, xyz_dir, metalq_reference, train_ratio=0.8, val_ratio=0.1):
    """
    金属種ごとに層化してデータ分割
    ...
    for mol_id in tqdm(mol_ids, desc="金属種特定"):
        ...
        atoms, _ = read_xyz_file(xyz_path)
    ...
    for metal, count in sorted_metals[:15]:
...
def main():
    ...
    # ===== Metal_q参照値ロード =====
    print("\n🔹 Metal_q参照値をロード中...")
    metalq_reference = load_metalq_reference(METAL_Q_REFERENCE_PATH)
    if metalq_reference:
        print(f"✓ {len(metalq_reference)} エントリ（重複含む）を読み込みました")
    else:
        print("❌ エラー: Metal_q参照値の読み込みに失敗しました")
        sys.exit(1)
    ...
    atoms, positions = read_xyz_file(sample_xyz)
    
    # Metal_q参照値から取得（CSVからのみ）
    metalq_ref = metalq_reference.get(mol_ids[0], None)
    
    if atoms is not None:
        print(f"  サンプル分子: {mol_ids[0]}")
        print(f"    原子数: {len(atoms)}")
        print(f"    Metal_q（CSV参照値）: {metalq_ref if metalq_ref else 'N/A'}")
        print(f"    原子種: {set(atoms)}")
        
        if metalq_ref is None:
            print(f"\n⚠️  警告: サンプル分子のMetal_qが取得できません")
            print(f"  デバッグ情報:")
            print(f"    - mol_id: {mol_ids[0]}")
            print(f"    - metalq_reference内: {mol_ids[0] in metalq_reference}")
            print(f"    - 参照値サンプル: {list(metalq_reference.items())[:3]}")
    else:
    ...
    train_mol_ids, val_mol_ids, test_mol_ids, metal_counts = stratified_split_by_metal(
        mol_ids, XYZ_DIR, metalq_reference, 
    ...
    train_graphs_baseline = create_molecular_graphs_baseline(
        train_mol_ids, XYZ_DIR, metalq_reference, "tmQM_Baseline_Train"
    )
    val_graphs_baseline = create_molecular_graphs_baseline(
        val_mol_ids, XYZ_DIR, metalq_reference, "tmQM_Baseline_Val"
    )
    test_graphs_baseline = create_molecular_graphs_baseline(
        test_mol_ids, XYZ_DIR, metalq_reference, "tmQM_Baseline_Test"
    )
    ...
    train_graphs_pfp = create_molecular_graphs_pfp(
        train_mol_ids, descriptor_data, XYZ_DIR, metalq_reference, "tmQM_PFP_Train"
    )
    val_graphs_pfp = create_molecular_graphs_pfp(
        val_mol_ids, descriptor_data, XYZ_DIR, metalq_reference, "tmQM_PFP_Val"
    )
    test_graphs_pfp = create_molecular_graphs_pfp(
        test_mol_ids, descriptor_data, XYZ_DIR, metalq_reference, "tmQM_PFP_Test"
    )
    ...
    if len(train_graphs_baseline) == 0 and len(train_graphs_pfp) == 0:
        ...
        print(f"  metalq_reference内: {test_mol in metalq_reference}")
        
        if test_mol in metalq_reference:
            print(f"  Metal_q値: {metalq_reference[test_mol]}")
        
        atoms, pos = read_xyz_file(test_xyz)
        if atoms:
            print(f"  原子数: {len(atoms)}")
            print(f"  原子種: {set(atoms)}")
            ...
            graph = create_graph_from_xyz_baseline(
                test_xyz, test_mol, metalq_reference, cutoff=5.0, debug=True
            )
            if graph:
                print(f"    ✓ グラフ作成成功")
            else:
    ...
    for graph in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline:
        all_metalq_baseline.append(graph.y.item())
    
    for graph in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp:
        all_metalq_pfp.append(graph.y.item())
    
    all_metalq_baseline = np.array(all_metalq_baseline)
    all_metalq_pfp = np.array(all_metalq_pfp)
    ...
    stats_baseline = {
        ...
        'n_molecules_success': len(all_metalq_baseline),
        'success_rate': f"{len(all_metalq_baseline)/n_total*100:.1f}%",
        'split_config': {
        ...
        'metalq_statistics': {
            'total': {
                'min': float(all_metalq_baseline.min()),
                'max': float(all_metalq_baseline.max()),
                'mean': float(all_metalq_baseline.mean()),
                'std': float(all_metalq_baseline.std())
            }
        }
    }
    ...
    stats_pfp = {
        ...
        'target': 'Metal_q',
        'n_molecules_total': n_total,
        'n_molecules_success': len(all_metalq_pfp),
        'success_rate': f"{len(all_metalq_pfp)/n_total*100:.1f}%",
        'split_config': {
            'train': len(train_mol_ids),
            'val': len(val_mol_ids),
            'test': len(test_mol_ids)
        ...
        'metalq_statistics': {
            'total': {
                'min': float(all_metalq_pfp.min()),
                'max': float(all_metalq_pfp.max()),
                'mean': float(all_metalq_pfp.mean()),
                'std': float(all_metalq_pfp.std())
            }
        }
    }
    ...
    # ===== 最終サマリー =====
    print("\n" + "="*60)
    print("✅ tmQM 金属錯体 Metal_q用グラフ作成完了!")
    print("="*60)
    ...
    print(f"  Metal_q範囲: {stats_baseline['metalq_statistics']['total']['min']:.4f} - {stats_baseline['metalq_statistics']['total']['max']:.4f}")
    print(f"  Metal_q平均: {stats_baseline['metalq_statistics']['total']['mean']:.4f} ± {stats_baseline['metalq_statistics']['total']['std']:.4f}")
    ...
    print(f"  Metal_q範囲: {stats_pfp['metalq_statistics']['total']['min']:.4f} - {stats_pfp['metalq_statistics']['total']['max']:.4f}")
    print(f"  Metal_q平均: {stats_pfp['metalq_statistics']['total']['mean']:.4f} ± {stats_pfp['metalq_statistics']['total']['std']:.4f}")
    
    print(f"\n{'='*60}")
    print(f"🎉 完了 - 2種類のグラフデータセット（金属錯体・Metal_q）を作成しました")
    print(f"{'='*60}")

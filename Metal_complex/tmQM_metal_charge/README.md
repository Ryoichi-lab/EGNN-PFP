...
| 構成 | MAE |
|---|---|
| baseline (EGNN) | 0.0562 |
| PFP記述子付き (EGNN×PFP) | 0.0196 |
...
| `01_make_graph_metalq.py` | baseline/PFP両方のグラフ(PyG Data)を構築 |
| `02_train_egnn_pfp_metalq.py` | EGNN×PFP 学習（PFP記述子のみ付加、BOなし版） |
| `03_train_egnn_baseline_metalq.py` | EGNN baseline 学習 |
| `04_add_bond_order_to_graph_metalq.py` | PFPグラフに bond order エッジ特徴量を追加 |
| `05_train_egnn_pfp_bo_metalq.py` | EGNN×PFP+BO 学習（最終モデル） |
| `06_compare_baseline_vs_pfp_metalq.py` | baseline/PFPモデルの誤差分布比較 |
| `07_evaluate_pfp_metalq.py` | PFPモデルの性能評価 |
| `08_compare_plot_metalq.py` | baseline vs PFP+BO の比較プロット作成（results/の図を生成） |

グラフ構築・学習スクリプトは他のtmQM物性(μ, Δε, εHOMO, εLUMO)と共通の構造（同一スクリプト群を物性ごとにコピーして利用）です。

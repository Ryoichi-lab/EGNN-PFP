...
"""
tmQM HOMO予測 - Baseline vs PFP+BO 比較プロット
"""
...
class EGNN_Baseline_HOMO(nn.Module):
    """EGNN Baseline for HOMO"""
    ...
    def forward(self, batch):
        ...
        homo_pred = self.homo_head(graph_embedding)
        
        return {
            'homo_energy': homo_pred,
            'node_embeddings': h,
...
class EGNN_PFP_BO_Homo(nn.Module):
    """EGNN with PFP and Bond Order for HOMO"""
...
def evaluate_model(model, dataloader, device, convert_to_ev=True):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            # Baselineは'homo_energy', PFP+BOは'homo'キーを使用
            if 'homo_energy' in outputs:
                preds = outputs['homo_energy'].view(-1).cpu().numpy()
            else:
                preds = outputs['homo'].view(-1).cpu().numpy()
...
def main():
    ...
    BASELINE_MODEL_PATH = os.path.join(TMQM_DIR, "training_baseline_egnn_homo2/best_baseline_model.pth")
    BASELINE_GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_baseline_homo2")
    
    PFP_BO_MODEL_PATH = os.path.join(TMQM_DIR, "training_pfp_bo_egnn_homov2_0122/best_model_bo.pth")
    PFP_BO_GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_pfp_homo_bo2")
    
    OUTPUT_DIR = os.path.join(TMQM_DIR, "comparison_plots_homo")
    ...
    # =============================================
    print("\n" + "="*70)
    print("📊 Baseline Model Evaluation (HOMO)")
    print("="*70)
    ...
    # モデル読み込み
    baseline_model = EGNN_Baseline_HOMO(input_dim=5, hidden_dim=128, num_layers=7).to(device)
    ...
    # =============================================
    print("\n" + "="*70)
    print("📊 PFP+BO Model Evaluation (HOMO)")
    print("="*70)
    ...
    # モデル読み込み
    pfp_bo_model = EGNN_PFP_BO_Homo(input_dim=261, hidden_dim=128, 
...
# =============================================
    ...
    ax1.set_xlabel('True HOMO Energy (eV)', fontsize=24, fontweight='bold')
    ax1.set_ylabel('Predicted HOMO Energy (eV)', fontsize=24, fontweight='bold')
    ...
    ax2.set_xlabel('True HOMO Energy (eV)', fontsize=24, fontweight='bold')
    ax2.set_ylabel('Predicted HOMO Energy (eV)', fontsize=24, fontweight='bold')
    ...
    # 保存パスの定義
    plot_path = os.path.join(OUTPUT_DIR, 'baseline_vs_pfp_bo_homo_comparison.png')
...
# =============================================
    ...
    ax1.set_xlabel('True Dipole Moment (D)', fontsize=24, fontweight='bold')
    ax1.set_ylabel('Predicted Dipole Moment (D)', fontsize=24, fontweight='bold')
    ax1.set_title('Baseline', fontsize=26, fontweight='bold', pad=20)
    ax1.tick_params(axis='both', which='major', labelsize=20)
    ax1.grid(True, alpha=0.3, linewidth=1.2)
    ax1.set_xlim(plot_min, plot_max)
    ...
    ax2.set_xlabel('True Dipole Moment (D)', fontsize=24, fontweight='bold')
    ax2.set_ylabel('Predicted Dipole Moment (D)', fontsize=24, fontweight='bold')
    ax2.set_title('EGNN-PFP', fontsize=26, fontweight='bold', pad=20)
    ax2.tick_params(axis='both', which='major', labelsize=20)
    ax2.grid(True, alpha=0.3, linewidth=1.2)
    ax2.set_xlim(plot_min, plot_max)
    ...
    # 保存パスの定義
    plot_path = os.path.join(OUTPUT_DIR, 'baseline_vs_pfp_bo_comparison.png')
    ...
    print("🎉 Comparison Complete!")
    print("="*70)
    print(f"Baseline MAE:  {baseline_mae:.4f} D  (R²: {baseline_r2:.4f})")
    print(f"PFP+BO MAE:    {pfp_bo_mae:.4f} D  (R²: {pfp_bo_r2:.4f})")
    print(f"Improvement:   {improvement:.1f}%")
    print("="*70 + "\n")
    ...
    ax.set_xlabel('True HOMO Energy (eV)', fontsize=38, fontweight='bold')
    ax.set_ylabel('Predicted HOMO Energy (eV)', fontsize=38, fontweight='bold')
    ...
    # 図のラベルを追加（左上）
    ax.text(0.05, 0.95, '(c) HOMO Energy', transform=ax.transAxes,

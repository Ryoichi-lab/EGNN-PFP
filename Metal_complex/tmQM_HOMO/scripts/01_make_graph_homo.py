...
HOMO_REFERENCE_PATH = "/home/users/uchiyama/tmQM_dipole/datasets/tmqm/tmqm/tmQM/tmQM_y.csv"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(TMQM_BASE_DIR, "graphs_tmQM_baseline_homo2")
OUTPUT_DIR_PFP = os.path.join(TMQM_BASE_DIR, "graphs_tmQM_pfp_homo2")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("tmQMデータセット 金属錯体 HOMO用グラフ作成")
print("EGNN-PFP 金属錯体拡張版")
print("="*60)
print("\n目的:")
print("  1. EGNNベースライン（PFPなし）")
print("  2. EGNN×PFP（PFP記述子あり）")
print("  の性能比較（金属錯体・HOMO）")
...
def load_HOMO_reference(reference_path):
    """
    tmQM_y.csvからHOMOの参照値を読み込む
    
    フォーマット: CSD_code;Electronic_E;Dispersion_E;Dipole_M;Metal_q;HL_Gap;HOMO_Energy;HOMO_Energy;...
    ...
    try:
        ...
        # HOMO_Energyカラムが存在するか確認
        if 'HOMO_Energy' not in df.columns:
            print(f"⚠️ 警告: 'HOMO_Energy'カラムが見つかりません")
            return {}
        ...
        for idx, row in df.iterrows():
            ...
            HOMO_Energy = row['HOMO_Energy']
            
            # NaNチェック
            if pd.isna(csd_code) or pd.isna(HOMO_Energy):
                continue
            
            # 両方の形式で登録
            HOMO_dict[csd_code] = HOMO_Energy
            HOMO_dict[f"{csd_code}.xyz"] = HOMO_Energy
        
        print(f"✓ {len(df)} 分子のHOMO参照値を読み込みました")
        print(f"  HOMO範囲: {df['HOMO_Energy'].min():.4f} - {df['HOMO_Energy'].max():.4f} eV")
        
        # サンプル表示（.xyz形式）
        sample_items = [(k, v) for k, v in list(HOMO_dict.items())[:6] if k.endswith('.xyz')]
        print(f"  サンプル (.xyz形式): {sample_items[:3]}")
        
        return HOMO_dict
        
    except Exception as e:
        print(f"⚠️ HOMO参照値の読み込みエラー: {e}")
        import traceback
        traceback.print_exc()
        return {}
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, HOMO_reference=None, cutoff=5.0, debug=False):
    """EGNNベースライン用：XYZファイルからグラフデータを作成（PFPなし、HOMO用）"""
    try:
        ...
        # HOMOの取得（CSVからのみ）
        HOMO_Energy = None
        if HOMO_reference and mol_id in HOMO_reference:
            HOMO_Energy = HOMO_reference[mol_id]
        
        if HOMO_Energy is None:
            if debug:
                print(f"  {mol_id}: HOMOが取得できません（CSVに存在しない）")
            return None
        ...
        return Data(
            ...
            y=torch.FloatTensor([HOMO_Energy]),
            mol_id=mol_id
        )
    except Exception as e:
        if debug:
...
# ===== PFP版：グラフ作成関数 =====
def create_graph_from_xyz_pfp(xyz_path, pfp_descriptors, mol_id, HOMO_reference=None, cutoff=5.0, debug=False):
    """PFP版：XYZファイルとPFP記述子からグラフデータを作成（HOMO用）"""
    try:
        atoms, positions = read_xyz_file(xyz_path)
...
# ===== グラフデータ作成関数（ベースライン用） =====
def create_molecular_graphs_baseline(mol_ids, xyz_dir, HOMO_reference, split_name):
    """ベースライン用：グラフデータ作成（PFPなし、HOMO用）"""
    graph_data_list = []
    failed_molecules = []
...
# ===== グラフデータ作成関数（PFP版） =====
def create_molecular_graphs_pfp(mol_ids, descriptor_data, xyz_dir, HOMO_reference, split_name):
    """PFP版：グラフデータ作成（HOMO用）"""
    graph_data_list = []
    failed_molecules = []
...
# ===== データ分割（金属種による層化分割） =====
def stratified_split_by_metal(mol_ids, xyz_dir, HOMO_reference, train_ratio=0.8, val_ratio=0.1):
    """
    金属種ごとに層化してデータ分割
...
def main():
    ...
    # ===== HOMO参照値ロード =====
    print("\n🔹 HOMO参照値をロード中...")
    HOMO_reference = load_HOMO_reference(HOMO_REFERENCE_PATH)
    if HOMO_reference:
        print(f"✓ {len(HOMO_reference)} エントリ（重複含む）を読み込みました")
    else:
        print("❌ エラー: HOMO参照値の読み込みに失敗しました")
        sys.exit(1)
    ...
    # HOMO参照値から取得（CSVからのみ）
    HOMO_ref = HOMO_reference.get(mol_ids[0], None)
    
    if atoms is not None:
        print(f"  サンプル分子: {mol_ids[0]}")
        print(f"    原子数: {len(atoms)}")
        print(f"    HOMO（CSV参照値）: {HOMO_ref if HOMO_ref else 'N/A'} eV")
        print(f"    原子種: {set(atoms)}")
        
        if HOMO_ref is None:
            print(f"\n⚠️  警告: サンプル分子のHOMOが取得できません")
            print(f"  デバッグ情報:")
            print(f"    - mol_id: {mol_ids[0]}")
            print(f"    - HOMO_reference内: {mol_ids[0] in HOMO_reference}")
            print(f"    - 参照値サンプル: {list(HOMO_reference.items())[:3]}")
    else:
    ...
    train_mol_ids, val_mol_ids, test_mol_ids, metal_counts = stratified_split_by_metal(
        mol_ids, XYZ_DIR, HOMO_reference, 
    ...
    train_graphs_baseline = create_molecular_graphs_baseline(
        train_mol_ids, XYZ_DIR, HOMO_reference, "tmQM_Baseline_Train"
    )
    val_graphs_baseline = create_molecular_graphs_baseline(
        val_mol_ids, XYZ_DIR, HOMO_reference, "tmQM_Baseline_Val"
    )
    test_graphs_baseline = create_molecular_graphs_baseline(
        test_mol_ids, XYZ_DIR, HOMO_reference, "tmQM_Baseline_Test"
    )
    ...
    train_graphs_pfp = create_molecular_graphs_pfp(
        train_mol_ids, descriptor_data, XYZ_DIR, HOMO_reference, "tmQM_PFP_Train"
    )
    val_graphs_pfp = create_molecular_graphs_pfp(
        val_mol_ids, descriptor_data, XYZ_DIR, HOMO_reference, "tmQM_PFP_Val"
    )
    test_graphs_pfp = create_molecular_graphs_pfp(
        test_mol_ids, descriptor_data, XYZ_DIR, HOMO_reference, "tmQM_PFP_Test"
    )
    ...
    if len(train_graphs_baseline) == 0 and len(train_graphs_pfp) == 0:
        ...
        print(f"  HOMO_reference内: {test_mol in HOMO_reference}")
        
        if test_mol in HOMO_reference:
            print(f"  HOMO値: {HOMO_reference[test_mol]}")
        ...
        if atoms:
            ...
            graph = create_graph_from_xyz_baseline(
                test_xyz, test_mol, HOMO_reference, cutoff=5.0, debug=True
            )
            if graph:
                print(f"    ✓ グラフ作成成功")
            else:
    ...
    for graph in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline:
        all_HOMO_baseline.append(graph.y.item())
    
    for graph in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp:
        all_HOMO_pfp.append(graph.y.item())
    
    all_HOMO_baseline = np.array(all_HOMO_baseline)
    all_HOMO_pfp = np.array(all_HOMO_pfp)
    ...
    stats_baseline = {
        ...
        'n_molecules_success': len(all_HOMO_baseline),
        'success_rate': f"{len(all_HOMO_baseline)/n_total*100:.1f}%",
        'split_config': {
        ...
        'HOMO_statistics': {
            'total': {
                'min': float(all_HOMO_baseline.min()),
                'max': float(all_HOMO_baseline.max()),
                'mean': float(all_HOMO_baseline.mean()),
                'std': float(all_HOMO_baseline.std())
            }
        }
    }
    ...
    stats_pfp = {
        ...
        'target': 'HOMO_Energy',
        'n_molecules_total': n_total,
        'n_molecules_success': len(all_HOMO_pfp),
        'success_rate': f"{len(all_HOMO_pfp)/n_total*100:.1f}%",
        'split_config': {
            'train': len(train_mol_ids),
            'val': len(val_mol_ids),
            'test': len(test_mol_ids)
        ...
        'HOMO_statistics': {
            'total': {
                'min': float(all_HOMO_pfp.min()),
                'max': float(all_HOMO_pfp.max()),
                'mean': float(all_HOMO_pfp.mean()),
                'std': float(all_HOMO_pfp.std())
            }
        }
    }
    ...
    print(f"  HOMO範囲: {stats_baseline['HOMO_statistics']['total']['min']:.4f} - {stats_baseline['HOMO_statistics']['total']['max']:.4f} eV")
    print(f"  HOMO平均: {stats_baseline['HOMO_statistics']['total']['mean']:.4f} ± {stats_baseline['HOMO_statistics']['total']['std']:.4f} eV")
    ...
    print(f"  HOMO範囲: {stats_pfp['HOMO_statistics']['total']['min']:.4f} - {stats_pfp['HOMO_statistics']['total']['max']:.4f} eV")
    print(f"  HOMO平均: {stats_pfp['HOMO_statistics']['total']['mean']:.4f} ± {stats_pfp['HOMO_statistics']['total']['std']:.4f} eV")
    
    print(f"\n{'='*60}")
    print(f"🎉 完了 - 2種類のグラフデータセット（金属錯体・HOMO）を作成しました")
    print(f"{'='*60}")

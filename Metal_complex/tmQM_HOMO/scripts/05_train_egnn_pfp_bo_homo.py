...
# =============================================
# EGNN×PFP+BO モデル（HOMO用）
# =============================================

class EGNN_PFP_BO_Homo(nn.Module):
    """
    EGNN×PFP+BO - HOMOエネルギー予測用
...
def plot_predictions(predictions, targets, epoch, mae, output_dir, split_name="Val"):
    ...
    plt.xlabel('True HOMO Energy (eV)', fontsize=14, fontweight='bold')
    plt.ylabel('Predicted HOMO Energy (eV)', fontsize=14, fontweight='bold')
...
def main():
    ...
    GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_pfp_homo_bo2")
    OUTPUT_DIR = os.path.join(TMQM_DIR, "training_pfp_bo_egnn_homov3_0124")
    ...
    print("="*70)
    print("tmQM 金属錯体 HOMO予測 - EGNN×PFP+BO")
    ...
    print(f"  ターゲット: {sample.y.shape} (HOMO Energy)")
    ...
    model = EGNN_PFP_BO_Homo(input_dim=261, hidden_dim=128, edge_dim=5, num_layers=7).to(device)
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training - EGNN×PFP+BO (HOMO)")
    print(f"{'='*70}\n")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae:
            ...
            checkpoint = {
                ...
                'val_mae': val_mae,
                'history': history,
                'model_type': 'egnn_pfp_bo_tmqm_homo'
            }
    ...
    ax1.set_title('EGNN×PFP+BO (HOMO) - MAE', fontsize=14, fontweight='bold')

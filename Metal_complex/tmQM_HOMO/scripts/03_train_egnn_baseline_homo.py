...
"""
tmQM 金属錯体 HOMO予測 - EGNN Baseline
...
# =============================================
# EGNN Baseline モデル（HOMO用）
# =============================================

class EGNN_Baseline_HOMO(nn.Module):
    """
    EGNN Baseline - HOMO予測用（PFPなし）
...
def train_epoch(model, dataloader, optimizer, device):
    ...
    for batch in tqdm(dataloader, desc="Training", leave=False):
        ...
        outputs = model(batch)
        homo_pred = outputs['homo_energy'].view(-1)
        homo_true = batch.y.view(-1)
        
        # MSE Loss（論文準拠）
        loss = nn.MSELoss()(homo_pred, homo_true)
        mae = torch.mean(torch.abs(homo_pred - homo_true))
...
def validate_epoch(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Validation", leave=False):
            ...
            all_predictions.append(homo_pred.cpu().numpy())
            all_targets.append(homo_true.cpu().numpy())
...
def main():
    ...
    GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_baseline_homo2")
    OUTPUT_DIR = os.path.join(TMQM_DIR, "training_baseline_egnn_homo3")
    ...
    print("="*70)
    print("tmQM 金属錯体 HOMO予測 - EGNN Baseline")
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training - tmQM Baseline (HOMO, No PFP)")
    print(f"{'='*70}\n")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae:
            ...
            checkpoint = {
                ...
                'val_mae': val_mae,
                'history': history,
                'model_type': 'egnn_baseline_tmqm_homo',
    ...
    ax1.set_title('EGNN Baseline (tmQM HOMO) - MAE', fontsize=14, fontweight='bold')
    ...
    print(f"\n{'='*70}")
    print("🎉 EGNN Baseline Training Complete (tmQM HOMO)!")
    print(f"{'='*70}")

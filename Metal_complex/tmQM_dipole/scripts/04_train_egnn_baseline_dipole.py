...
"""
tmQM 金属錯体 双極子モーメント予測 - EGNN Baseline
...
class EGNN_Baseline(nn.Module):
    """
    EGNN Baseline - EGNN原著論文準拠（PFPなし）
...
def train_epoch(model, dataloader, optimizer, device):
    ...
    for batch in tqdm(dataloader, desc="Training", leave=False):
        ...
        outputs = model(batch)
        dipole_pred = outputs['molecular_dipole'].view(-1)
        dipole_true = batch.y.view(-1)
        
        # MSE Loss（論文準拠）
        loss = nn.MSELoss()(dipole_pred, dipole_true)
        mae = torch.mean(torch.abs(dipole_pred - dipole_true))
...
def validate_epoch(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Validation", leave=False):
            ...
            all_predictions.append(dipole_pred.cpu().numpy())
            all_targets.append(dipole_true.cpu().numpy())
...
def main():
    ...
    GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_baseline2")
    OUTPUT_DIR = os.path.join(TMQM_DIR, "training_baseline_egnn_dipole3")
    ...
    print("="*70)
    print("tmQM 金属錯体 双極子予測 - EGNN Baseline")
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training - tmQM Baseline (No PFP)")
    print(f"{'='*70}\n")
    ...
    ax1.set_title('EGNN Baseline (tmQM) - MAE', fontsize=14, fontweight='bold')
    ...
    print(f"\n{'='*70}")
    print("🎉 EGNN Baseline Training Complete (tmQM Metal Complexes)!")
    print(f"{'='*70}")

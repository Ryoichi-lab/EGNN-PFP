...
"""
tmQM 双極子予測 - Baseline vs PFP+BO 比較プロット
"""
...
class EGNN_Baseline(nn.Module):
    ...
    def __init__(self, input_dim=5, hidden_dim=128, num_layers=7):
        ...
        self.egnn_layers = nn.ModuleList([
            EGNNLayerBaseline(hidden_dim=hidden_dim)
...
def evaluate_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            outputs = model(batch)
            preds = outputs['molecular_dipole'].view(-1).cpu().numpy()
...
def main():
    ...
    BASELINE_MODEL_PATH = os.path.join(TMQM_DIR, "training_baseline_egnn_dipole2/best_baseline_model.pth")
    BASELINE_GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_baseline2")
    
    PFP_BO_MODEL_PATH = os.path.join(TMQM_DIR, "training_pfp_bo_egnn_v2_0120/best_model_bo.pth")
    PFP_BO_GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_pfp_dipole_bo2")
    
    OUTPUT_DIR = os.path.join(TMQM_DIR, "comparison_plots_0130_dipole")
    ...
    # モデル読み込み
    baseline_model = EGNN_Baseline(input_dim=5, hidden_dim=128, num_layers=7).to(device)
    ...
    # =============================================
    print("\n" + "="*70)
    print("📊 PFP+BO Model Evaluation")
    print("="*70)
    
    # データ読み込み
    pfp_bo_test = torch.load(os.path.join(PFP_BO_GRAPH_DIR, "test_graphs.pt"))
    pfp_bo_loader = DataLoader(pfp_bo_test, batch_size=32, shuffle=False)
    
    # モデル読み込み
    pfp_bo_model = EGNN_PFP_BO(input_dim=261, hidden_dim=128, 
    ...
    print(f"✓ PFP+BO MAE: {pfp_bo_mae:.4f} D")
...
# =============================================
    ...
    # 保存パスの定義
    plot_path = os.path.join(OUTPUT_DIR, 'baseline_vs_pfp_bo_dipole_comparison.png')
    ...
    ax.set_xlabel('True (D)', fontsize=38, fontweight='bold')
    ax.set_ylabel('Predicted (D)', fontsize=38, fontweight='bold')

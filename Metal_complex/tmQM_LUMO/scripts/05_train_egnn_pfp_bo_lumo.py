...
# =============================================
# EGNN×PFP+BO モデル（LUMO用）
# =============================================

class EGNN_PFP_BO_LUMO(nn.Module):
    """
    EGNN×PFP+BO - LUMOエネルギー予測用
...
def plot_predictions(predictions, targets, epoch, mae, output_dir, split_name="Val"):
    ...
    plt.xlabel('True LUMO Energy (eV)', fontsize=14, fontweight='bold')
    plt.ylabel('Predicted LUMO Energy (eV)', fontsize=14, fontweight='bold')
...
def main():
    ...
    GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_pfp_lumo_bo2")  # ✓ 正しいスペル
    OUTPUT_DIR = os.path.join(TMQM_DIR, "training_pfp_bo_egnn_lumo3")
    ...
    print("="*70)
    print("tmQM 金属錯体 LUMO予測 - EGNN×PFP+BO")
    ...
    print(f"  ターゲット: {sample.y.shape} (LUMO Energy)")
    ...
    model = EGNN_PFP_BO_LUMO(input_dim=261, hidden_dim=128, edge_dim=5, num_layers=7).to(device)
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training - EGNN×PFP+BO (LUMO)")
    print(f"{'='*70}\n")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae:
            ...
            checkpoint = {
                ...
                'val_mae': val_mae,
                'history': history,
                'model_type': 'egnn_pfp_bo_tmqm_LUMO'
            }
    ...
    ax1.set_title('EGNN×PFP+BO (LUMO) - MAE', fontsize=14, fontweight='bold')
    ...
    ax4.set_xlabel('True LUMO (eV)', fontsize=12, fontweight='bold')
    ax4.set_ylabel('Predicted LUMO (eV)', fontsize=12, fontweight='bold')

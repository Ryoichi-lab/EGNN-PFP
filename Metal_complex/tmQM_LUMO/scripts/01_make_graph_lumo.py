...
LUMO_REFERENCE_PATH = "/home/users/uchiyama/tmQM_dipole/datasets/tmqm/tmqm/tmQM/tmQM_y.csv"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(TMQM_BASE_DIR, "graphs_tmQM_baseline_lumo2")
OUTPUT_DIR_PFP = os.path.join(TMQM_BASE_DIR, "graphs_tmQM_pfp_lumo2")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("tmQMデータセット 金属錯体 LUMO用グラフ作成")
print("EGNN-PFP 金属錯体拡張版")
print("="*60)
print("\n目的:")
print("  1. EGNNベースライン（PFPなし）")
print("  2. EGNN×PFP（PFP記述子あり）")
print("  の性能比較（金属錯体・LUMO）")
...
def load_lumo_reference(reference_path):
    """
    tmQM_y.csvからLUMOの参照値を読み込む
    ...
    try:
        ...
        # LUMO_Energyカラムが存在するか確認
        if 'LUMO_Energy' not in df.columns:
            print(f"⚠️ 警告: 'LUMO_Energy'カラムが見つかりません")
            return {}
        ...
        for idx, row in df.iterrows():
            ...
            lumo_energy = row['LUMO_Energy']
            
            # NaNチェック
            if pd.isna(csd_code) or pd.isna(lumo_energy):
                continue
            
            # 両方の形式で登録
            lumo_dict[csd_code] = lumo_energy
            lumo_dict[f"{csd_code}.xyz"] = lumo_energy
        
        print(f"✓ {len(df)} 分子のLUMO参照値を読み込みました")
        print(f"  LUMO範囲: {df['LUMO_Energy'].min():.4f} - {df['LUMO_Energy'].max():.4f} eV")
        
        # サンプル表示（.xyz形式）
        sample_items = [(k, v) for k, v in list(lumo_dict.items())[:6] if k.endswith('.xyz')]
        print(f"  サンプル (.xyz形式): {sample_items[:3]}")
        
        return lumo_dict
        
    except Exception as e:
        print(f"⚠️ LUMO参照値の読み込みエラー: {e}")
        import traceback
        traceback.print_exc()
        return {}
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, lumo_reference=None, cutoff=5.0, debug=False):
    """EGNNベースライン用：XYZファイルからグラフデータを作成（PFPなし、LUMO用）"""
    try:
        ...
        # LUMOの取得（CSVからのみ）
        lumo_energy = None
        if lumo_reference and mol_id in lumo_reference:
            lumo_energy = lumo_reference[mol_id]
        
        if lumo_energy is None:
            if debug:
                print(f"  {mol_id}: LUMOが取得できません（CSVに存在しない）")
            return None
        ...
        return Data(
            ...
            y=torch.FloatTensor([lumo_energy]),
            mol_id=mol_id
        )
    except Exception as e:
        if debug:
...
# ===== PFP版：グラフ作成関数 =====
def create_graph_from_xyz_pfp(xyz_path, pfp_descriptors, mol_id, lumo_reference=None, cutoff=5.0, debug=False):
    """PFP版：XYZファイルとPFP記述子からグラフデータを作成（LUMO用）"""
    try:
        atoms, positions = read_xyz_file(xyz_path)
...
# ===== グラフデータ作成関数（ベースライン用） =====
def create_molecular_graphs_baseline(mol_ids, xyz_dir, lumo_reference, split_name):
    """ベースライン用：グラフデータ作成（PFPなし、LUMO用）"""
    graph_data_list = []
    failed_molecules = []
...
# ===== グラフデータ作成関数（PFP版） =====
def create_molecular_graphs_pfp(mol_ids, descriptor_data, xyz_dir, lumo_reference, split_name):
    """PFP版：グラフデータ作成（LUMO用）"""
    graph_data_list = []
    failed_molecules = []
...
# ===== データ分割（金属種による層化分割） =====
def stratified_split_by_metal(mol_ids, xyz_dir, lumo_reference, train_ratio=0.8, val_ratio=0.1):
    """
    金属種ごとに層化してデータ分割
...
def main():
    ...
    # ===== LUMO参照値ロード =====
    print("\n🔹 LUMO参照値をロード中...")
    lumo_reference = load_lumo_reference(LUMO_REFERENCE_PATH)
    if lumo_reference:
        print(f"✓ {len(lumo_reference)} エントリ（重複含む）を読み込みました")
    else:
        print("❌ エラー: LUMO参照値の読み込みに失敗しました")
        sys.exit(1)
    ...
    # LUMO参照値から取得（CSVからのみ）
    lumo_ref = lumo_reference.get(mol_ids[0], None)
    
    if atoms is not None:
        print(f"  サンプル分子: {mol_ids[0]}")
        print(f"    原子数: {len(atoms)}")
        print(f"    LUMO（CSV参照値）: {lumo_ref if lumo_ref else 'N/A'} eV")
        print(f"    原子種: {set(atoms)}")
        
        if lumo_ref is None:
            print(f"\n⚠️  警告: サンプル分子のLUMOが取得できません")
            print(f"  デバッグ情報:")
            print(f"    - mol_id: {mol_ids[0]}")
            print(f"    - lumo_reference内: {mol_ids[0] in lumo_reference}")
            print(f"    - 参照値サンプル: {list(lumo_reference.items())[:3]}")
    else:
    ...
    train_mol_ids, val_mol_ids, test_mol_ids, metal_counts = stratified_split_by_metal(
        mol_ids, XYZ_DIR, lumo_reference, 
    ...
    train_graphs_baseline = create_molecular_graphs_baseline(
        train_mol_ids, XYZ_DIR, lumo_reference, "tmQM_Baseline_Train"
    )
    val_graphs_baseline = create_molecular_graphs_baseline(
        val_mol_ids, XYZ_DIR, lumo_reference, "tmQM_Baseline_Val"
    )
    test_graphs_baseline = create_molecular_graphs_baseline(
        test_mol_ids, XYZ_DIR, lumo_reference, "tmQM_Baseline_Test"
    )
    ...
    train_graphs_pfp = create_molecular_graphs_pfp(
        train_mol_ids, descriptor_data, XYZ_DIR, lumo_reference, "tmQM_PFP_Train"
    )
    val_graphs_pfp = create_molecular_graphs_pfp(
        val_mol_ids, descriptor_data, XYZ_DIR, lumo_reference, "tmQM_PFP_Val"
    )
    test_graphs_pfp = create_molecular_graphs_pfp(
        test_mol_ids, descriptor_data, XYZ_DIR, lumo_reference, "tmQM_PFP_Test"
    )
    ...
    if len(train_graphs_baseline) == 0 and len(train_graphs_pfp) == 0:
        ...
        print(f"  lumo_reference内: {test_mol in lumo_reference}")
        
        if test_mol in lumo_reference:
            print(f"  LUMO値: {lumo_reference[test_mol]}")
        ...
        if atoms:
            ...
            graph = create_graph_from_xyz_baseline(
                test_xyz, test_mol, lumo_reference, cutoff=5.0, debug=True
            )
            if graph:
                print(f"    ✓ グラフ作成成功")
            else:
    ...
    for graph in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline:
        all_lumo_baseline.append(graph.y.item())
    
    for graph in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp:
        all_lumo_pfp.append(graph.y.item())
    
    all_lumo_baseline = np.array(all_lumo_baseline)
    all_lumo_pfp = np.array(all_lumo_pfp)
    ...
    stats_baseline = {
        ...
        'n_molecules_success': len(all_lumo_baseline),
        'success_rate': f"{len(all_lumo_baseline)/n_total*100:.1f}%",
        'split_config': {
        ...
        'lumo_statistics': {
            'total': {
                'min': float(all_lumo_baseline.min()),
                'max': float(all_lumo_baseline.max()),
                'mean': float(all_lumo_baseline.mean()),
                'std': float(all_lumo_baseline.std())
            }
        }
    }
    ...
    stats_pfp = {
        ...
        'target': 'LUMO_Energy',
        'n_molecules_total': n_total,
        'n_molecules_success': len(all_lumo_pfp),
        'success_rate': f"{len(all_lumo_pfp)/n_total*100:.1f}%",
        'split_config': {
            'train': len(train_mol_ids),
            'val': len(val_mol_ids),
            'test': len(test_mol_ids)
        ...
        'lumo_statistics': {
            'total': {
                'min': float(all_lumo_pfp.min()),
                'max': float(all_lumo_pfp.max()),
                'mean': float(all_lumo_pfp.mean()),
                'std': float(all_lumo_pfp.std())
            }
        }
    }
    ...
    print(f"  LUMO範囲: {stats_baseline['lumo_statistics']['total']['min']:.4f} - {stats_baseline['lumo_statistics']['total']['max']:.4f} eV")
    print(f"  LUMO平均: {stats_baseline['lumo_statistics']['total']['mean']:.4f} ± {stats_baseline['lumo_statistics']['total']['std']:.4f} eV")
    ...
    print(f"  LUMO範囲: {stats_pfp['lumo_statistics']['total']['min']:.4f} - {stats_pfp['lumo_statistics']['total']['max']:.4f} eV")
    print(f"  LUMO平均: {stats_pfp['lumo_statistics']['total']['mean']:.4f} ± {stats_pfp['lumo_statistics']['total']['std']:.4f} eV")
    
    print(f"\n{'='*60}")
    print(f"🎉 完了 - 2種類のグラフデータセット（金属錯体・LUMO）を作成しました")
    print(f"{'='*60}")

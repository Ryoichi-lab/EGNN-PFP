...
"""
tmQM HOMO-LUMO Gap Prediction - PFP Model Performance
"""
...
def main():
    ...
    # モデル設定
    checkpoint_path = os.path.join(TMQM_DIR, "training_pfp_egnn_homolumo/best_pfp_model.pth")
    ...
    # モデルインスタンス作成
    model = EGNN_PFP_HomoLumo().to(device)
    
    # チェックポイント読み込み
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
    model.load_state_dict(checkpoint['model_state_dict'])
    print(f"✓ エポック {checkpoint.get('epoch', 'N/A')}, Val MAE: {checkpoint.get('val_mae', 'N/A'):.4f} Hartree")
    ...
    min_val = trues.min()
    max_val = trues.max()
    plt.plot([min_val, max_val], [min_val, max_val], 'k--', lw=2.5)
    ...
    plt.title("HOMO-LUMO Gap Prediction - EGNN×PFP (tmQM)", fontsize=19, fontweight='bold')
    plt.grid(alpha=0.3)
    plt.tick_params(axis='both', which='major', labelsize=15)
    plt.tight_layout()

    output_path_yy = os.path.join(OUTPUT_DIR, "pfp_homolumo_yy_plot.png")
    ...
    print("結果サマリー")
    print(f"{'='*60}")
    print(f"EGNN×PFP: MAE = {mae:.4f} eV, R² = {r2:.4f}")
    print(f"{'='*60}\n")

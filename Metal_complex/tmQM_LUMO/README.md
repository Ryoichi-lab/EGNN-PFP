...
| `01_make_graph_lumo.py` | baseline/PFP両方のグラフ(PyG Data)を構築 |
| `02_train_egnn_pfp_lumo.py` | EGNN×PFP 学習（PFP記述子のみ付加、BOなし版） |
| `03_train_egnn_baseline_lumo.py` | EGNN baseline 学習 |
| `04_add_bond_order_to_graph_lumo.py` | PFPグラフに bond order エッジ特徴量を追加 |
| `05_train_egnn_pfp_bo_lumo.py` | EGNN×PFP+BO 学習（最終モデル） |
| `06_compare_baseline_vs_pfp_lumo.py` | baseline/PFPモデルの誤差分布比較 |
| `07_evaluate_pfp_lumo.py` | PFPモデルの性能評価 |
| `08_compare_plot_lumo.py` | baseline vs PFP+BO の比較プロット作成（results/の図を生成） |

グラフ構築・学習スクリプトは他のtmQM物性(μ, Δε, εHOMO, Metal_q)と共通の構造（同一スクリプト群を物性ごとにコピーして利用）です。

...
"""
tmQM 金属錯体 HOMO-LUMOギャップ予測 - EGNN×PFP+BO
...
# =============================================
# EGNN×PFP+BO モデル（HOMO-LUMOギャップ用）
# =============================================

class EGNN_PFP_BO_HomoLumo(nn.Module):
    """
    EGNN×PFP+BO - HOMO-LUMOギャップ予測用
...
def main():
    ...
    GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_pfp_homolumo_bo_added2")
    OUTPUT_DIR = os.path.join(TMQM_DIR, "training_pfp_bo_egnn_homolumo_v3_0124")
    ...
    print("="*70)
    print("tmQM 金属錯体 HOMO-LUMOギャップ予測 - EGNN×PFP+BO")
    ...
    model = EGNN_PFP_BO_HomoLumo(input_dim=261, hidden_dim=128, edge_dim=5, num_layers=7).to(device)
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training - EGNN×PFP+BO (HOMO-LUMO Gap)")
    print(f"{'='*70}\n")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae:
            ...
            checkpoint = {
                ...
                'val_mae': val_mae,
                'history': history,
                'model_type': 'egnn_pfp_bo_tmqm_homolumo'
            }
    ...
    ax1.set_title('EGNN×PFP+BO (HOMO-LUMO) - MAE', fontsize=14, fontweight='bold')

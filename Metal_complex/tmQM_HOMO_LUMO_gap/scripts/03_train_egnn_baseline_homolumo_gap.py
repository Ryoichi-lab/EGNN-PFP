...
"""
tmQM 金属錯体 HOMO-LUMOギャップ予測 - EGNN Baseline
...
# =============================================
# EGNN Baseline モデル（HOMO-LUMOギャップ用）
# =============================================
...
def train_epoch(model, dataloader, optimizer, device):
    ...
    for batch in tqdm(dataloader, desc="Training", leave=False):
        ...
        outputs = model(batch)
        gap_pred = outputs['homolumo_gap'].view(-1)
        gap_true = batch.y.view(-1)
        
        # MSE Loss（論文準拠）
        loss = nn.MSELoss()(gap_pred, gap_true)
        mae = torch.mean(torch.abs(gap_pred - gap_true))
...
def validate_epoch(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Validation", leave=False):
            ...
            all_predictions.append(gap_pred.cpu().numpy())
            all_targets.append(gap_true.cpu().numpy())
...
def plot_predictions(predictions, targets, epoch, mae, output_dir, split_name="Val"):
    ...
    plt.xlabel('True HOMO-LUMO Gap (eV)', fontsize=14, fontweight='bold')
    plt.ylabel('Predicted HOMO-LUMO Gap (eV)', fontsize=14, fontweight='bold')
...
def main():
    ...
    GRAPH_DIR = os.path.join(TMQM_DIR, "graphs_tmQM_baseline_homolumo2")
    OUTPUT_DIR = os.path.join(TMQM_DIR, "training_baseline_egnn_homolumo2")
    ...
    print("="*70)
    print("tmQM 金属錯体 HOMO-LUMOギャップ予測 - EGNN Baseline")
    ...
    print(f"  HOMO-LUMOギャップ: {sample.y.shape}")
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training - tmQM Baseline (HOMO-LUMO Gap, No PFP)")
    print(f"{'='*70}\n")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae:
            ...
            checkpoint = {
                ...
                'val_mae': val_mae,
                'history': history,
                'model_type': 'egnn_baseline_tmqm_homolumo',
    ...
    ax1.set_title('EGNN Baseline (tmQM HOMO-LUMO) - MAE', fontsize=14, fontweight='bold')
    ...
    print(f"\n{'='*70}")
    print("🎉 EGNN Baseline Training Complete (tmQM HOMO-LUMO Gap)!")
    print(f"{'='*70}")

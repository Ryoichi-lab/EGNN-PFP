...
# XYZファイルとHL_Gap参照値のパス
XYZ_DIR = "/home/users/uchiyama/tmQM_dipole/datasets/tmqm/tmqm/tmQM/xyz_files"
HLGAP_REFERENCE_PATH = "/home/users/uchiyama/tmQM_dipole/datasets/tmqm/tmqm/tmQM/tmQM_y.csv"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(TMQM_BASE_DIR, "graphs_tmQM_baseline_homolumo2")
OUTPUT_DIR_PFP = os.path.join(TMQM_BASE_DIR, "graphs_tmQM_pfp_homolumo2")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("tmQMデータセット 金属錯体 HOMO-LUMOギャップ用グラフ作成")
print("EGNN-PFP 金属錯体拡張版")
print("="*60)
print("\n目的:")
print("  1. EGNNベースライン（PFPなし）")
print("  2. EGNN×PFP（PFP記述子あり）")
print("  の性能比較（金属錯体・HOMO-LUMOギャップ）")
print(f"\n作業ディレクトリ: {TMQM_BASE_DIR}")


# ===== XYZファイルからHOMO-LUMOギャップを読み取る関数 =====
def read_xyz_with_homolumo(xyz_path):
    """XYZファイルから原子座標を読み取る（tmQM形式対応）"""
    try:
...
def load_homolumo_reference(reference_path):
    """
    tmQM_y.csvからHOMO-LUMOギャップの参照値を読み込む
    
    フォーマット: CSD_code;Electronic_E;Dispersion_E;Dipole_M;Metal_q;HL_Gap;...
    ...
    try:
        ...
        # HL_Gapカラムが存在するか確認
        if 'HL_Gap' not in df.columns:
            print(f"⚠️ 警告: 'HL_Gap'カラムが見つかりません")
            return {}
        ...
        for idx, row in df.iterrows():
            ...
            # NaNチェック
            if pd.isna(csd_code) or pd.isna(hl_gap):
                continue
            
            # 両方の形式で登録
            hlgap_dict[csd_code] = hl_gap
            hlgap_dict[f"{csd_code}.xyz"] = hl_gap
        
        print(f"✓ {len(df)} 分子のHOMO-LUMOギャップ参照値を読み込みました")
        print(f"  HOMO-LUMOギャップ範囲: {df['HL_Gap'].min():.4f} - {df['HL_Gap'].max():.4f} eV")
        
        # サンプル表示（.xyz形式）
        sample_items = [(k, v) for k, v in list(hlgap_dict.items())[:6] if k.endswith('.xyz')]
        print(f"  サンプル (.xyz形式): {sample_items[:3]}")
        
        return hlgap_dict
        
    except Exception as e:
        print(f"⚠️ HOMO-LUMOギャップ参照値の読み込みエラー: {e}")
        import traceback
        traceback.print_exc()
        return {}
    
# ===== ベースライン用：PFPなしのグラフ作成関数（修正版） =====
def create_graph_from_xyz_baseline(xyz_path, mol_id, hlgap_reference=None, cutoff=5.0, debug=False):
    """EGNNベースライン用：XYZファイルからグラフデータを作成（PFPなし、HOMO-LUMOギャップ用）"""
    try:
        atoms, positions = read_xyz_with_homolumo(xyz_path)
        ...
        # HOMO-LUMOギャップの取得（CSVからのみ）
        hl_gap = None
        if hlgap_reference and mol_id in hlgap_reference:
            hl_gap = hlgap_reference[mol_id]
        
        if hl_gap is None:
            if debug:
                print(f"  {mol_id}: HOMO-LUMOギャップが取得できません（CSVに存在しない）")
            return None
...
# ===== PFP版：グラフ作成関数（修正版） =====
def create_graph_from_xyz_pfp(xyz_path, pfp_descriptors, mol_id, hlgap_reference=None, cutoff=5.0, debug=False):
    """PFP版：XYZファイルとPFP記述子からグラフデータを作成（HOMO-LUMOギャップ用）"""
    try:
        atoms, positions = read_xyz_with_homolumo(xyz_path)
        ...
        return None
# ===== グラフデータ作成関数（ベースライン用） =====
def create_molecular_graphs_baseline(mol_ids, xyz_dir, hlgap_reference, split_name):
    """ベースライン用：グラフデータ作成（PFPなし、HOMO-LUMOギャップ用）"""
    graph_data_list = []
    failed_molecules = []
    ...
    for mol_id in tqdm(mol_ids):
        ...
        graph_data = create_graph_from_xyz_baseline(
            xyz_path=xyz_path,
            mol_id=mol_id,
            hlgap_reference=hlgap_reference
        )
...
# ===== グラフデータ作成関数（PFP版） =====
def create_molecular_graphs_pfp(mol_ids, descriptor_data, xyz_dir, hlgap_reference, split_name):
    """PFP版：グラフデータ作成（HOMO-LUMOギャップ用）"""
    graph_data_list = []
    failed_molecules = []
...
# ===== データ分割（金属種による層化分割） =====
def stratified_split_by_metal(mol_ids, xyz_dir, hlgap_reference, train_ratio=0.8, val_ratio=0.1):
    """
    金属種ごとに層化してデータ分割
    ...
    for mol_id in tqdm(mol_ids, desc="金属種特定"):
        ...
        atoms, _ = read_xyz_with_homolumo(xyz_path)  # 修正：戻り値が2つ
...
def main():
    ...
    # ===== HOMO-LUMOギャップ参照値ロード =====
    print("\n🔹 HOMO-LUMOギャップ参照値をロード中...")
    hlgap_reference = load_homolumo_reference(HLGAP_REFERENCE_PATH)
    if hlgap_reference:
        print(f"✓ {len(hlgap_reference)} エントリ（重複含む）を読み込みました")
    else:
        print("❌ エラー: HOMO-LUMOギャップ参照値の読み込みに失敗しました")
        sys.exit(1)
...
# ===== サンプルデータ確認 =====
    ...
    atoms, positions = read_xyz_with_homolumo(sample_xyz)  # 修正：戻り値が2つ
    
    # HOMO-LUMOギャップ参照値から取得（CSVからのみ）
    hl_gap_ref = hlgap_reference.get(mol_ids[0], None)
    
    if atoms is not None:
        print(f"  サンプル分子: {mol_ids[0]}")
        print(f"    原子数: {len(atoms)}")
        print(f"    HOMO-LUMOギャップ（CSV参照値）: {hl_gap_ref if hl_gap_ref else 'N/A'} eV")
        print(f"    原子種: {set(atoms)}")
        
        if hl_gap_ref is None:
            print(f"\n⚠️  警告: サンプル分子のHOMO-LUMOギャップが取得できません")
            print(f"  デバッグ情報:")
            print(f"    - mol_id: {mol_ids[0]}")
            print(f"    - hlgap_reference内: {mol_ids[0] in hlgap_reference}")
            print(f"    - 参照値サンプル: {list(hlgap_reference.items())[:3]}")
    else:
    ...
    train_mol_ids, val_mol_ids, test_mol_ids, metal_counts = stratified_split_by_metal(
        mol_ids, XYZ_DIR, hlgap_reference, 
    ...
    train_graphs_baseline = create_molecular_graphs_baseline(
        train_mol_ids, XYZ_DIR, hlgap_reference, "tmQM_Baseline_Train"
    )
    val_graphs_baseline = create_molecular_graphs_baseline(
        val_mol_ids, XYZ_DIR, hlgap_reference, "tmQM_Baseline_Val"
    )
    test_graphs_baseline = create_molecular_graphs_baseline(
        test_mol_ids, XYZ_DIR, hlgap_reference, "tmQM_Baseline_Test"
    )
    ...
    train_graphs_pfp = create_molecular_graphs_pfp(
        train_mol_ids, descriptor_data, XYZ_DIR, hlgap_reference, "tmQM_PFP_Train"
    )
    val_graphs_pfp = create_molecular_graphs_pfp(
        val_mol_ids, descriptor_data, XYZ_DIR, hlgap_reference, "tmQM_PFP_Val"
    )
    test_graphs_pfp = create_molecular_graphs_pfp(
        test_mol_ids, descriptor_data, XYZ_DIR, hlgap_reference, "tmQM_PFP_Test"
    )
    ...
    if len(train_graphs_baseline) == 0 and len(train_graphs_pfp) == 0:
        ...
        print(f"  hlgap_reference内: {test_mol in hlgap_reference}")
        
        if test_mol in hlgap_reference:
            print(f"  HOMO-LUMOギャップ値: {hlgap_reference[test_mol]}")
        
        atoms, pos = read_xyz_with_homolumo(test_xyz) 
        if atoms:
            print(f"  原子数: {len(atoms)}")
            print(f"  原子種: {set(atoms)}")
            print(f"  XYZから読み取ったHOMO-LUMOギャップ: {hl_gap}")
            ...
            graph = create_graph_from_xyz_baseline(
                test_xyz, test_mol, hlgap_reference, cutoff=5.0, debug=True
            )
            if graph:
                print(f"    ✓ グラフ作成成功")
            else:
            ...
            if test_mol in descriptor_data:
                ...
                graph_pfp = create_graph_from_xyz_pfp(
                    ...
                    hlgap_reference, cutoff=5.0, debug=True
                )
                if graph_pfp:
                    print(f"    ✓ グラフ作成成功")
                else:
    ...
    # ===== HOMO-LUMOギャップ統計情報を収集 =====
    print("\n🔹 HOMO-LUMOギャップ統計情報を収集中...")
    
    all_hlgap_baseline = []
    all_hlgap_pfp = []
    
    for graph in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline:
        all_hlgap_baseline.append(graph.y.item())
    
    for graph in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp:
        all_hlgap_pfp.append(graph.y.item())
    
    all_hlgap_baseline = np.array(all_hlgap_baseline)
    all_hlgap_pfp = np.array(all_hlgap_pfp)
    ...
    stats_baseline = {
        ...
        'n_molecules_success': len(all_hlgap_baseline),
        'success_rate': f"{len(all_hlgap_baseline)/n_total*100:.1f}%",
        'split_config': {
            'train': len(train_mol_ids),
            'val': len(val_mol_ids),
            'test': len(test_mol_ids)
        },
        'random_seed': 42,
        ...
        'homolumo_statistics': {
            'total': {
                'min': float(all_hlgap_baseline.min()),
                'max': float(all_hlgap_baseline.max()),
                'mean': float(all_hlgap_baseline.mean()),
                'std': float(all_hlgap_baseline.std())
            }
        }
    }
    ...
    stats_pfp = {
        ...
        'target': 'homolumo_gap',
        'n_molecules_total': n_total,
        'n_molecules_success': len(all_hlgap_pfp),
        'success_rate': f"{len(all_hlgap_pfp)/n_total*100:.1f}%",
        'split_config': {
            'train': len(train_mol_ids),
            'val': len(val_mol_ids),
            'test': len(test_mol_ids)
        ...
        'homolumo_statistics': {
            'total': {
                'min': float(all_hlgap_pfp.min()),
                'max': float(all_hlgap_pfp.max()),
                'mean': float(all_hlgap_pfp.mean()),
                'std': float(all_hlgap_pfp.std())
            }
        }
    }
    ...
    print(f"  HOMO-LUMOギャップ範囲: {stats_baseline['homolumo_statistics']['total']['min']:.4f} - {stats_baseline['homolumo_statistics']['total']['max']:.4f} eV")
    print(f"  HOMO-LUMOギャップ平均: {stats_baseline['homolumo_statistics']['total']['mean']:.4f} ± {stats_baseline['homolumo_statistics']['total']['std']:.4f} eV")
    ...
    print(f"  HOMO-LUMOギャップ範囲: {stats_pfp['homolumo_statistics']['total']['min']:.4f} - {stats_pfp['homolumo_statistics']['total']['max']:.4f} eV")
    print(f"  HOMO-LUMOギャップ平均: {stats_pfp['homolumo_statistics']['total']['mean']:.4f} ± {stats_pfp['homolumo_statistics']['total']['std']:.4f} eV")
    
    print(f"\n{'='*60}")
    print(f"🎉 完了 - 2種類のグラフデータセット（金属錯体・HOMO-LUMOギャップ）を作成しました")
    print(f"{'='*60}")

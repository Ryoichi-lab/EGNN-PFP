...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'dipole_pfp_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def main():
    ...
    GRAPH_DIR = os.path.join(QM9_DIPOLE_DIR, "graphs_dipole_qm9_B3LYP_pfp_perfect")
    OUTPUT_DIR = os.path.join(QM9_DIPOLE_DIR, "training_pfp_B3LYP_egnn_official_128_attention_perfect_0115")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP 双極子予測 - EGNN×PFP")

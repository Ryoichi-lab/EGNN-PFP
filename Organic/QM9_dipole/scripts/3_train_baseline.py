...
"""
QM9 B3LYP 双極子モーメント予測 - EGNN Baseline
EGNN公式実装の忠実な再現（PFPなし）+ Early Stopping
ベースライン専用グラフを使用（PFP版グラフからもone-hot抽出可能）
"""
...
class EGNN_Official(nn.Module):
    ...
    def forward(self, h0, x, edges, edge_attr, node_mask, edge_mask, n_nodes):
        ...
        Args:
            ...
            edge_attr: エッジ特徴 [num_edges, in_edge_nf] or None
...
def prepare_batch_official_baseline(batch, device):
    ...
    create_atom_onehotを使用してone-hot特徴を抽出
    """
    ...
    # ★create_atom_onehotを使用してone-hot特徴を作成
    atom_features = create_atom_onehot(batch, device)
    
    h0 = torch.zeros(total_nodes, 5, device=device)  # one-hot: 5次元
...
def train_epoch_official(model, loader, optimizer, device):
    """公式実装形式での訓練"""
    model.train()
    total_loss = 0
    total_mae = 0
...
def validate_epoch_official(model, loader, device):
    """公式実装形式での検証"""
    model.eval()
    total_mae = 0
    num_samples = 0
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    ax1.set_xlabel('True Dipole (D)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted Dipole (D)', fontsize=12, fontweight='bold')
    r2_stat = r2_score(targets, predictions)
    ax1.set_title(f'EGNN Baseline - Epoch {epoch}\nMAE: {mae:.4f} D, R²: {r2_stat:.4f}', 
    ...
    ax2.set_xlabel('Prediction Error (D)', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Frequency', fontsize=12, fontweight='bold')
    ax2.set_title(f'Error Distribution\nMean: {errors.mean():.4f} D, Std: {errors.std():.4f} D', 
                  fontsize=14, fontweight='bold')
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'dipole_baseline_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def plot_training_history(history, output_dir):
    ...
    ax1.axhline(y=0.029, color='red', linestyle='--', linewidth=2, label='Paper EGNN')
...
def main():
    ...
    GRAPH_DIR = os.path.join(QM9_DIPOLE_DIR, "graphs_dipole_qm9_B3LYP_baseline_perfect")
    OUTPUT_DIR = os.path.join(QM9_DIPOLE_DIR, "training_baseline_dipole_B3LYP_0115")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP 双極子モーメント予測 - EGNN Baseline")
    print("EGNN公式実装（PFPなし）+ カットオフ5Å + Early Stopping")
    print("="*70)
    ...
    print(f"  エッジ特徴: {sample.edge_attr.shape if hasattr(sample, 'edge_attr') else 'None'}")
    print(f"  座標: {sample.pos.shape}")
    print(f"  双極子: {sample.y.shape}")
    ...
    # Early Stopping設定
    patience = 500
    min_improvement = 0.0001  # 0.0001 D
    
    print(f"\n{'='*70}")
    print("⚙️  EGNN公式実装準拠設定（Baseline: PFPなし）")
    print(f"{'='*70}")
    print(f"  Target:        Dipole moment")
    ...
    print(f"  Early Stop:    Patience={patience}, Min improvement={min_improvement} D")
    print(f"{'='*70}")
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_mae = train_epoch_official(model, train_loader, optimizer, device)
        val_mae, val_preds, val_targets = validate_epoch_official(model, val_loader, device)
        ...
        if val_mae < best_val_mae - min_improvement:
            ...
            if epoch == 0:
                ...
                print(f"  ⭐ New best! Improved by {improvement:.4f} D (Patience reset)")
        ...
        if patience_counter >= patience:
            ...
            print(f"    Best Val MAE: {best_val_mae:.4f} D (Epoch {best_epoch})")
            break
    ...
    print(f"Paper EGNN MAE: {PAPER_MAE} D")

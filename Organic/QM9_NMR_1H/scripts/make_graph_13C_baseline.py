...
# -*- coding: utf-8 -*-
"""
QM9NMR データセット用グラフ作成スクリプト（¹³C NMR, Baseline）
- ノード特徴: one-hot元素(5) + 距離特徴(4) = 9次元（PFPなし）
- エッジ: 完全グラフ（カットオフなし）
- エッジ特徴: [距離] = 1次元（PFPなし）
- ターゲット: 炭素原子の¹³Cシールディング定数（気相, 1列目）
...
OUTPUT_DIR   = os.path.join(BASE_DIR, "EGNN_PFP/graphs/13C_baseline")
...
print("=" * 60)
print("QM9NMR ¹³C NMR グラフ作成（Baseline: PFPなし）")
...
def parse_nmr_file(filepath, target_element, solvent_col):
    """
    SI_DFT_NMR.txt をパース。
    返り値: {mol_name: {'elements': [...], 'shielding': [...]}}
    """
    print(f"\n[1/3] NMRデータをパース中: {filepath}")
    mol_data = {}
...
def build_node_features(positions, atoms):
    ...
    for i in range(len(positions)):
        ...
        feat = np.concatenate([
            oh,                                              # 5 (one-hot)
            [np.linalg.norm(positions[i] - center),          # dist to center
             np.min(dists) if dists else 0.0,                # min dist
             np.mean(dists) if dists else 0.0,               # mean dist
             float(np.sum(np.array(dists) < 3.0))]          # coordination
        ])
        feats.append(feat)
...
def create_graph(mol_key, xyz_dir, nmr_data):
    ...
    edge_index, edge_feats = build_edges_fully_connected(positions)

    # ターゲット: 対象原子のみ値、それ以外は -1
    ...
    return Data(
        ...
        edge_attr  = torch.FloatTensor(edge_feats) if edge_feats else torch.zeros((0, 1)),
        y          = y,
        c_mask     = mask,
        mol_id     = mol_key
    )
...
# 2. 共通分子を抽出して5-fold分割（各fold独立保存）
print(f"\n[2/3] データ分割 (5-fold KFold: 各fold独立保存)")
all_keys = np.array(sorted(nmr_data.keys()))
print(f"  総分子数: {len(all_keys)}")
...
def build_graphs(keys, split_name):
    ...
    for mol_key in tqdm(keys, desc=split_name):
        g = create_graph(mol_key, XYZ_DIR, nmr_data)
        if g is not None:
            graphs.append(g)
        else:
...
total_target  = sum(g.c_mask.sum().item() for g in all_graphs)
all_shielding = np.concatenate([g.y[g.c_mask].numpy() for g in all_graphs])
...
stats = {
    ...
    'edge_feature_dim': 1,
    'descriptor':       'None (baseline: no PFP)',
    'n_common_molecules': len(all_keys),
...
print(f"\n  ノード特徴次元: {stats['node_feature_dim']} (PFPなし baseline)")
print(f"  エッジ特徴次元: {stats['edge_feature_dim']} (距離のみ)")

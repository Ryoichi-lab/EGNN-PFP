...
"""
QM9NMR ¹H NMR遮蔽定数予測 - EGNN-PFP 5-Fold Cross Validation
...
def train_one_fold(val_fold, graph_dir, output_base, device, num_epochs=500):
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae - min_improve:
            ...
            if (epoch + 1) % 50 == 0:
                plot_predictions(val_preds, val_targets, epoch+1, val_mae, fold_dir, '1H')
        else:
            patience_counter += 1
...
def main():
    ...
    OUTPUT_BASE = os.path.expanduser("~/qm9nmr/EGNN_PFP/training_1H_cv")
    ...
    print("=" * 70)
    print("QM9NMR ¹H NMR遮蔽定数予測 - EGNN-PFP  5-Fold CV")
    ...
    print(f"\n{'='*70}")
    print("  5-Fold CV 結果サマリー  ¹H NMR PFP")
    print(f"{'='*70}")

...
# -*- coding: utf-8 -*-
"""
QM9NMR データセット用グラフ作成スクリプト（¹³C NMR）
...
OUTPUT_DIR   = os.path.join(BASE_DIR, "EGNN_PFP/graphs/13C")
...
def parse_nmr_file(filepath, target_element, solvent_col):
    ...
    while i < len(lines):
        ...
        if line.isdigit():
            n_atoms   = int(line)
            mol_name  = lines[i + 1].strip()          # e.g. dsgdb9_000001
            mol_key   = mol_name + ".xyz"              # e.g. dsgdb9_000001.xyz
            target_shielding = []

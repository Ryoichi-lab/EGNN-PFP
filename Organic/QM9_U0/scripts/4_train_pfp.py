...
"""
QM9 B3LYP U0 (Internal energy at 0K) 予測 - EGNN×PFP
EGNN公式実装の忠実な再現 + PFP記述子 + ターゲット標準化
単位: meV
"""
...
def compute_mean_mad(loader, device):
    ...
    print("  Collecting U0 target values...")
    for batch in loader:
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    ax1.set_xlabel('True U0 (meV)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted U0 (meV)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'u0_pfp_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def main():
    ...
    GRAPH_DIR = "/home/users/uchiyama/QM9_U0/graphs_U0_qm9_B3LYP_pfp_perfect"
    OUTPUT_DIR = "/home/users/uchiyama/QM9_U0/QM9_U0_training_pfp_egnn_128_atteintion_perfect_0110"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP U0 (Internal energy at 0K) 予測 - EGNN×PFP")
    ...
    print(f"{'='*70}")
    print(f"  Target:        U0 (Internal energy at 0K)")
    ...
    print(f"Paper EGNN MAE (U0): {PAPER_MAE_MEV:.2f} meV")
    print(f"{'='*70}\n")

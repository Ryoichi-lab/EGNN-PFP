...
# ===== 絶対パス設定 =====
QM9_U0_DIR = "/home/users/uchiyama/QM9_U0"
XYZ_DIR = os.path.join(QM9_U0_DIR, "qm9_xyz_u0") # XYZファイルがあるフォルダ
DESCRIPTOR_B3LYP_PATH = "/home/users/uchiyama/QM9_dipole/pfp_descriptors_qm9_B3LYP.npz"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(QM9_U0_DIR, "graphs_U0_qm9_B3LYP_baseline_perfect")
OUTPUT_DIR_PFP = os.path.join(QM9_U0_DIR, "graphs_U0_qm9_B3LYP_pfp_perfect")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("QM9データセット B3LYP/6-31G(2df,p) U0 (Electronic spatial extent) 用グラフ作成")
print("EGNN論文準拠（Anderson et al. 2019 split: 100k/18k/13k）")
print("="*60)

# ===== XYZファイルから U0 を読み取る関数 =====
def read_xyz_with_U0(xyz_path):
    """
    XYZファイルから原子座標とU0を読み取る
    2行目に 'U0=...' が書かれている前提
    """
    try:
        ...
        # U0=... を抽出
        match = re.search(r'u0=([-\d\.Ee+]+)', comment_line)
        if match:
            U0_val = float(match.group(1))
        else:
        ...
        return atoms, np.array(positions), U0_val
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, cutoff=None):  # cutoffをNoneに
    try:
        atoms, positions, U0_val = read_xyz_with_U0(xyz_path)
        
        if atoms is None or positions is None or U0_val is None:
            return None
...
train_graphs_baseline = create_molecular_graphs_baseline(train_mol_ids, XYZ_DIR, "U0_Baseline_Train")
val_graphs_baseline = create_molecular_graphs_baseline(val_mol_ids, XYZ_DIR, "U0_Baseline_Val")
test_graphs_baseline = create_molecular_graphs_baseline(test_mol_ids, XYZ_DIR, "U0_Baseline_Test")

print("\n【2/2】EGNN×PFP グラフ生成中...")
train_graphs_pfp = create_molecular_graphs_pfp(train_mol_ids, descriptor_data, XYZ_DIR, "U0_PFP_Train")
val_graphs_pfp = create_molecular_graphs_pfp(val_mol_ids, descriptor_data, XYZ_DIR, "U0_PFP_Val")
test_graphs_pfp = create_molecular_graphs_pfp(test_mol_ids, descriptor_data, XYZ_DIR, "U0_PFP_Test")

# ===== 統計情報収集 =====
all_U0_baseline = [g.y.item() for g in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline]
all_U0_pfp = [g.y.item() for g in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp]

all_U0_baseline = np.array(all_U0_baseline)
all_U0_pfp = np.array(all_U0_pfp)
...
stats_baseline = {
    ...
    'target': 'U0 (Electronic spatial extent)',
    'unit': 'Bohr²',
    ...
    'random_seed': 42,
    'U0_statistics': {
        'min': float(all_U0_baseline.min()),
        'max': float(all_U0_baseline.max()),
        'mean': float(all_U0_baseline.mean()),
        'std': float(all_U0_baseline.std())
    }
}
...
stats_pfp = {
    ...
    'random_seed': 42,
    'U0_statistics': {
        'min': float(all_U0_pfp.min()),
        'max': float(all_U0_pfp.max()),
        'mean': float(all_U0_pfp.mean()),
        'std': float(all_U0_pfp.std())
    }
}
...
print("\n" + "="*60)
print("✅ QM9 B3LYP U0 グラフ作成完了!")
print("="*60)
print(f"\n【ベースライン】")
print(f"  保存先: {OUTPUT_DIR_BASELINE}")
print(f"  U0範囲: {stats_baseline['U0_statistics']['min']:.4f} - {stats_baseline['U0_statistics']['max']:.4f} Bohr²")
print(f"\n【EGNN×PFP】")
print(f"  保存先: {OUTPUT_DIR_PFP}")
print(f"  U0範囲: {stats_pfp['U0_statistics']['min']:.4f} - {stats_pfp['U0_statistics']['max']:.4f} Bohr²")
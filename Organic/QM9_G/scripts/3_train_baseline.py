...
"""
QM9 B3LYP G (Gibbs free energy at 298.15K) 予測 - ベースラインEGNN
...
def main():
    ...
    GRAPH_DIR = os.path.join(QM9_G_DIR, "graphs_g_qm9_B3LYP_baseline")
    OUTPUT_DIR = "/home/users/uchiyama/QM9_G/training_baseline_G_egnn_1217"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
    print("="*70)
    print("QM9 B3LYP G (Enthalpy at 298.15K) 予測 - ベースラインEGNN")
    ...
    print(f"{'='*70}")
    print(f"  Target:        G (Internal energy at 298.15K)")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae - min_improvement:
            ...
            torch.save(checkpoint, os.path.join(OUTPUT_DIR, 'best_baseline_official_model_g.pth'))
    ...
    ax1.set_title('Baseline EGNN (Official) - MAE (G)', fontsize=14, fontweight='bold')
    ...
    stats = {
        ...
        'target': 'G (Internal energy at 298.15K)',

...
# -*- coding: utf-8 -*-
"""
QM9 B3LYP G (Enthalpy at 298.15K) 予測 - EGNN×PFP
"""
...
def prepare_batch_official(batch, device):
    ...
    for i in range(batch_size):
        ...
        if edge_mask_graph.sum() > 0:
            ...
            edge_attr_list.append(graph_edge_attr.to(device))  # デバイスに転送
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    # plot_predictions関数内（471-472行目）
    ax1.set_xlabel('True G (meV)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted G (meV)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'pfp_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def main():
    ...
    GRAPH_DIR = "/home/users/uchiyama/QM9_G/graphs_g_qm9_B3LYP_pfp"
    OUTPUT_DIR = "/home/users/uchiyama/QM9_G/QM9_G_training_pfp_egnn_1218"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP G (Internal energy at 0K) 予測 - EGNN×PFP")
    print("EGNN公式実装の忠実な再現 + PFP記述子 + ターゲット標準化")
    print("単位: meV")
    print("="*70)
    ...
    print("\n🔹 モデル初期化...")
    model = EGNN_Official(
        in_node_nf=261,      # PFP256 + atom1 + geom4
        in_edge_nf=4,        # エッジ特徴
        hidden_nf=128,
        device=device,
        act_fn=nn.SiLU(),
    ...
    print(f"  Coord update:  Disabled (official implementation)")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae - min_improvement:
            ...
            checkpoint = {
                ...
                'mean': mean.item(),
                'mad': mad.item(),
                'model_type': 'egnn_pfp_official_h',
                'official_implementation': True,
                'target_normalized': True,
                'unit': 'meV'
            }
            torch.save(checkpoint, os.path.join(OUTPUT_DIR, 'best_pfp_official_h_model.pth'))
    ...
    print(f"Paper EGNN MAE (G): {PAPER_MAE_MEV:.2f} meV")
    ...
    ax1.set_title('EGNN×PFP (Official) - MAE (G)', fontsize=14, fontweight='bold')
    ...
    if len(val_preds) > 0 and len(val_targets) > 0:
        ...
        ax4.set_xlabel('True G (meV)', fontsize=12, fontweight='bold')
        ax4.set_ylabel('Predicted G (meV)', fontsize=12, fontweight='bold')
        ax4.set_title(f'EGNN×PFP Official Results (MAE: {best_val_mae:.2f} meV, R²: {r2_val:.4f})', 
              fontsize=14, fontweight='bold')
        ax4.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'training_history_pfp_official_h.png'), dpi=200)
    plt.close()
    ...
    stats = {
        ...
        'target': 'G (Internal energy at 0K)',
    ...
    with open(os.path.join(OUTPUT_DIR, 'training_stats_pfp_official.json'), 'w') as f:
        json.dump(stats, f, indent=2)
    
    print(f"\n{'='*70}")
    print("🎉 EGNN×PFP Official Training Complete!")
    print(f"{'='*70}")

...
"""
QM9 B3LYP u (Internal energy at 0K) 予測 - EGNN×PFP
...
def compute_mean_mad(loader, device):
    ...
    print("  Collecting u target values...")
    for batch in loader:
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'u_pfp_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def main():
    QM9_DIR = "/home/users/uchiyama/QM9_U"
    GRAPH_DIR = "/home/users/uchiyama/QM9_U/graphs_U_qm9_B3LYP_pfp_perfect"
    OUTPUT_DIR = "/home/users/uchiyama/QM9_U/QM9_u_training_pfp_egnn_128_attention_perfect_0110"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP u (Internal energy at 0K) 予測 - EGNN×PFP")
    ...
    print(f"{'='*70}")
    print(f"  Target:        u (Internal energy at 0K)")

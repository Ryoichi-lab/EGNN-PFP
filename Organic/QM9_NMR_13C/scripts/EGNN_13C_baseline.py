...
"""
QM9NMR ¹³C NMR遮蔽定数予測 - EGNN Baseline（PFPなし）
完全グラフ版: カットオフなし、分子全体（H, C, N, O, F）
正規化なし（原子レベル予測）
ノード特徴: one-hot元素(5) + 距離特徴(4) = 9次元
エッジ特徴: 距離(1) = 1次元
単位: ppm
"""
...
def plot_predictions(preds, targets, epoch, mae, output_dir, label='13C'):
    ...
    ax.set_title(f'EGNN-PFP  Epoch {epoch}\nMAE={mae:.4f} ppm  R²={r2:.4f}', fontsize=13)
...
def main():
    GRAPH_DIR  = os.path.expanduser("~/qm9nmr/EGNN_PFP/graphs/13C_baseline")
    OUTPUT_DIR = os.path.expanduser("~/qm9nmr/EGNN_PFP/training_13C_baseline")
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # ── fold構成（学習時に自由に変更可能）──
    # 80/20 (原著準拠): TRAIN_FOLDS=[0,1,2,3], VAL_FOLD=4
    # 60/20/20:         TRAIN_FOLDS=[0,1,2], VAL_FOLD=3  (fold4は別途テスト用)
    TRAIN_FOLDS = [0, 1, 2, 3]
    VAL_FOLD    = 4
    ...
    print("=" * 70)
    print("QM9NMR ¹³C NMR遮蔽定数予測 - EGNN Baseline（PFPなし）")
    print("完全グラフ / 全原子ノード / 正規化なし")
    print("=" * 70)
    ...
    print(f"  Train C atoms: {sum(g.c_mask.sum().item() for g in train_graphs):,}")

    sample = train_graphs[0]
    print(f"\n  ノード特徴: {sample.x.shape}")
    print(f"  エッジ特徴: {sample.edge_attr.shape}")

    # ── ターゲット統計（参考） ──
    all_y = np.concatenate([g.y[g.c_mask.bool()].numpy() for g in train_graphs])
    print(f"\n  ¹³C シールディング統計 (train):")
    print(f"    Mean={all_y.mean():.2f}  Std={all_y.std():.2f}  "
          f"Min={all_y.min():.2f}  Max={all_y.max():.2f} ppm")
    ...
    print(f"\n{'='*70}")
    print("  Target:     ¹³C NMR Shielding (Gas phase, mPW1PW91/6-311+G(2d,p))")
    print(f"  Epochs:     {num_epochs}  |  Batch: {batch_size}  |  LR: {initial_lr}")
    print(f"  Hidden: 128  |  Layers: 7  |  Attention: True  |  Norm: None")
    print(f"{'='*70}")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae - min_improve:
            ...
            if (epoch + 1) % 50 == 0:
                plot_predictions(val_preds, val_targets, epoch+1, val_mae, OUTPUT_DIR, '13C')
        else:
            patience_counter += 1
    ...
    # ── 最終統計 ──
    stats = {
        'target': '13C_NMR_shielding', 'solvent': 'Gas phase',
        'best_val_mae': float(best_val_mae),
        'model_config': {'in_node_nf': 9, 'in_edge_nf': 1, 'hidden_nf': 128,
                         'n_layers': 7, 'attention': True},
        'training_config': {'epochs': num_epochs, 'batch_size': batch_size,
                            'lr': initial_lr, 'weight_decay': 1e-8,

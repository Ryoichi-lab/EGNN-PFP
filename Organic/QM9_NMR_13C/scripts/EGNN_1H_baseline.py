...
"""
QM9NMR ¹H NMR遮蔽定数予測 - EGNN Baseline（PFPなし）
...
def prepare_batch(batch, device):
    ...
    for i, n in enumerate(node_counts):
        ...
        target_mask[s:s+n] = batch.h_mask[mask].to(device)   # ← h_mask
...
def main():
    GRAPH_DIR  = os.path.expanduser("~/qm9nmr/EGNN_PFP/graphs/1H_baseline")
    OUTPUT_DIR = os.path.expanduser("~/qm9nmr/EGNN_PFP/training_1H_baseline")
    ...
    print("=" * 70)
    print("QM9NMR ¹H NMR遮蔽定数予測 - EGNN Baseline（PFPなし）")
    ...
    print(f"  Train H atoms: {sum(g.h_mask.sum().item() for g in train_graphs):,}")
    ...
    # ── ターゲット統計（参考） ──
    all_y = np.concatenate([g.y[g.h_mask.bool()].numpy() for g in train_graphs])
    print(f"\n  ¹H シールディング統計 (train):")
    ...
    print(f"\n{'='*70}")
    print("  Target:     ¹H NMR Shielding (Gas phase, mPW1PW91/6-311+G(2d,p))")

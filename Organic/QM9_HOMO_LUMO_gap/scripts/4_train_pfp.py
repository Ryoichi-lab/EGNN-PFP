...
"""
QM9 B3LYP HOMO-LUMO gap予測 - EGNN×PFP
EGNN公式実装の忠実な再現 + PFP記述子 + Early Stopping
"""
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    ax1.set_xlabel('True HOMO-LUMO gap (eV)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted HOMO-LUMO gap (eV)', fontsize=12, fontweight='bold')
    r2_stat = r2_score(targets, predictions)
    ax1.set_title(f'EGNN×PFP Official - Epoch {epoch}\nMAE: {mae:.4f} eV, R²: {r2_stat:.4f}', 
    ...
    ax2.set_xlabel('Prediction Error (eV)', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Frequency', fontsize=12, fontweight='bold')
    ax2.set_title(f'Error Distribution\nMean: {errors.mean():.4f} eV, Std: {errors.std():.4f} eV', 
                  fontsize=14, fontweight='bold')
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'gap_pfp_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def plot_training_history(history, output_dir):
    ...
    ax1.axhline(y=0.043, color='red', linestyle='--', linewidth=2, label='Paper EGNN (0.043 eV)')
...
def main():
    # 絶対パス設定
    QM9_HOMOLUMO_DIR = "/home/users/uchiyama/QM9_HOMO_LUMO"
    GRAPH_DIR = os.path.join(QM9_HOMOLUMO_DIR, "graphs_HOMO_LUMO_qm9_B3LYP_pfp_perfect")
    OUTPUT_DIR = os.path.join(QM9_HOMOLUMO_DIR, "training_pfp_gap_egnn_official_128_perfect_attention_0110")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP HOMO-LUMO gap予測 - EGNN×PFP")
    print("EGNN公式実装の忠実な再現 + PFP記述子 + Early Stopping")
    print("="*70)
    ...
    print(f"  HOMO-LUMO gap: {sample.y.shape}")
    ...
    # Early Stopping設定
    patience = 500
    min_improvement = 0.0001  # 0.0001 eV
    
    print(f"\n{'='*70}")
    print("⚙️  EGNN公式実装準拠設定 + PFP記述子")
    print(f"{'='*70}")
    print(f"  Target:        HOMO-LUMO gap")
    ...
    print(f"  Layers:        7")
    print(f"  Early Stop:    Patience={patience}, Min improvement={min_improvement} eV")
    print(f"{'='*70}")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae - min_improvement:
            ...
            if epoch == 0:
                ...
                print(f"  ⭐ New best! Improved by {improvement:.4f} eV (Patience reset)")
    ...
    print(f"Paper EGNN MAE: {PAPER_MAE} eV")

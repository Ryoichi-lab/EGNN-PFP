...
# ===== 絶対パス設定 =====
QM9_HOMO_LUMO_DIR = "/home/users/uchiyama/QM9_HOMO_LUMO"
XYZ_DIR = os.path.join(QM9_HOMO_LUMO_DIR, "qm9_xyz_gap") # XYZファイルがあるフォルダ
DESCRIPTOR_B3LYP_PATH = "/home/users/uchiyama/QM9_dipole/pfp_descriptors_qm9_B3LYP.npz"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(QM9_HOMO_LUMO_DIR, "graphs_HOMO_LUMO_qm9_B3LYP_baseline_perfect")
OUTPUT_DIR_PFP = os.path.join(QM9_HOMO_LUMO_DIR, "graphs_HOMO_LUMO_qm9_B3LYP_pfp_perfect")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("QM9データセット B3LYP/6-31G(2df,p) HOMO_LUMO (Electronic spatial extent) 用グラフ作成")
print("EGNN論文準拠（Anderson et al. 2019 split: 100k/18k/13k）")
print("="*60)

# ===== XYZファイルから HOMO_LUMO を読み取る関数 =====
def read_xyz_with_HOMO_LUMO(xyz_path):
    """
    XYZファイルから原子座標とHOMO_LUMOを読み取る
    2行目に 'HOMO_LUMO=...' が書かれている前提
    """
    try:
        ...
        # HOMO_LUMO=... を抽出
        match = re.search(r'gap=([-\d\.Ee+]+)', comment_line)
        if match:
            HOMO_LUMO_val = float(match.group(1))
        else:
        ...
        return atoms, np.array(positions), HOMO_LUMO_val
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, cutoff=None):  # cutoffをNoneに
    try:
        atoms, positions, HOMO_LUMO_val = read_xyz_with_HOMO_LUMO(xyz_path)
        
        if atoms is None or positions is None or HOMO_LUMO_val is None:
            return None
        ...
        return Data(
            ...
            y=torch.FloatTensor([HOMO_LUMO_val]),
            mol_id=mol_id
        )
    except Exception as e:
...
train_graphs_baseline = create_molecular_graphs_baseline(train_mol_ids, XYZ_DIR, "HOMO_LUMO_Baseline_Train")
val_graphs_baseline = create_molecular_graphs_baseline(val_mol_ids, XYZ_DIR, "HOMO_LUMO_Baseline_Val")
test_graphs_baseline = create_molecular_graphs_baseline(test_mol_ids, XYZ_DIR, "HOMO_LUMO_Baseline_Test")

print("\n【2/2】EGNN×PFP グラフ生成中...")
train_graphs_pfp = create_molecular_graphs_pfp(train_mol_ids, descriptor_data, XYZ_DIR, "HOMO_LUMO_PFP_Train")
val_graphs_pfp = create_molecular_graphs_pfp(val_mol_ids, descriptor_data, XYZ_DIR, "HOMO_LUMO_PFP_Val")
test_graphs_pfp = create_molecular_graphs_pfp(test_mol_ids, descriptor_data, XYZ_DIR, "HOMO_LUMO_PFP_Test")

# ===== 統計情報収集 =====
all_HOMO_LUMO_baseline = [g.y.item() for g in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline]
all_HOMO_LUMO_pfp = [g.y.item() for g in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp]

all_HOMO_LUMO_baseline = np.array(all_HOMO_LUMO_baseline)
all_HOMO_LUMO_pfp = np.array(all_HOMO_LUMO_pfp)
...
stats_baseline = {
    ...
    'target': 'HOMO_LUMO (Electronic spatial extent)',
    'unit': 'Bohr²',
    ...
    'random_seed': 42,
    'HOMO_LUMO_statistics': {
        'min': float(all_HOMO_LUMO_baseline.min()),
        'max': float(all_HOMO_LUMO_baseline.max()),
        'mean': float(all_HOMO_LUMO_baseline.mean()),
        'std': float(all_HOMO_LUMO_baseline.std())
    }
}
...
stats_pfp = {
    ...
    'random_seed': 42,
    'HOMO_LUMO_statistics': {
        'min': float(all_HOMO_LUMO_pfp.min()),
        'max': float(all_HOMO_LUMO_pfp.max()),
        'mean': float(all_HOMO_LUMO_pfp.mean()),
        'std': float(all_HOMO_LUMO_pfp.std())
    }
}
...
print("\n" + "="*60)
print("✅ QM9 B3LYP HOMO_LUMO グラフ作成完了!")
print("="*60)
print(f"\n【ベースライン】")
print(f"  保存先: {OUTPUT_DIR_BASELINE}")
print(f"  HOMO_LUMO範囲: {stats_baseline['HOMO_LUMO_statistics']['min']:.4f} - {stats_baseline['HOMO_LUMO_statistics']['max']:.4f} Bohr²")
print(f"\n【EGNN×PFP】")
print(f"  保存先: {OUTPUT_DIR_PFP}")
print(f"  HOMO_LUMO範囲: {stats_pfp['HOMO_LUMO_statistics']['min']:.4f} - {stats_pfp['HOMO_LUMO_statistics']['max']:.4f} Bohr²")
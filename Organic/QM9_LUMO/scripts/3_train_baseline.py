...
"""
QM9 B3LYP HOMO予測 - EGNN Baseline
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    ax1.set_xlabel('True HOMO (eV)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted HOMO (eV)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'homo_baseline_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def main():
    # 絶対パス設定
    QM9_HOMO_DIR = "/home/users/uchiyama/QM9_LUMO"
    
    # ★PFP版のグラフを使用（ベースラインは特徴抽出時にone-hotのみ使う）
    GRAPH_DIR = "/home/users/uchiyama/QM9_LUMO/graphs_LUMO_qm9_B3LYP_baseline_perfect"
    OUTPUT_DIR = os.path.join(QM9_HOMO_DIR, "training_baseline_LUMO_perfect_0113")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP HOMO予測 - EGNN Baseline")

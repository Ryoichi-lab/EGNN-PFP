...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    ax1.set_xlabel('True LUMO (eV)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted LUMO (eV)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'LUMO_pfp_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def main():
    ...
    GRAPH_DIR = os.path.join(QM9_LUMO_DIR, "graphs_LUMO_qm9_B3LYP_pfp_perfect")
    OUTPUT_DIR = os.path.join(QM9_LUMO_DIR, "training_pfp_LUMO_egnn_official_128_attention_perfect_0110")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP LUMO予測 - EGNN×PFP")

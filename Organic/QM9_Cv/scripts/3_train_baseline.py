...
"""
QM9 B3LYP Cv (Heat capacity at 298.15K) 予測 - ベースラインEGNN
EGNN公式実装の忠実な再現（PFP記述子なし）
単位: cal/(mol·K)
"""
...
def plot_predictions_cv(predictions, targets, epoch, mae, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'cv_baseline_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def main():
    ...
    GRAPH_DIR = os.path.join(QM9_CV_DIR, "graphs_Cv_qm9_B3LYP_baseline_v2")
    OUTPUT_DIR = os.path.join(QM9_CV_DIR, "training_baseline_Cv_egnn_1216")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP Cv (Heat capacity at 298.15K) 予測 - ベースラインEGNN")
    print("EGNN公式実装の忠実な再現（PFP記述子なし）")
    print("単位: cal/(mol·K)")
    print("="*70)
    ...
    print(f"\n{'='*70}")
    print("🚀 Starting Training (Baseline EGNN - Cv)")
    print(f"{'='*70}\n")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae - min_improvement:
            ...
            torch.save(checkpoint, os.path.join(OUTPUT_DIR, 'best_cv_baseline_official_model.pth'))
    ...
    # 訓練完了
    print("\n" + "="*70)
    print("📊 Training Complete (Baseline - Cv)")
    print("="*70)
    ...
    ax1.set_title('Baseline EGNN (Official) - MAE (Cv)', fontsize=14, fontweight='bold')
    ...
    if len(val_preds) > 0 and len(val_targets) > 0:
        ...
        ax4.set_title(f'Baseline Official Results (MAE: {best_val_mae:.4f} cal/(mol·K), R²: {r2_val:.4f})', 
                    fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'training_history_cv_baseline_official.png'), dpi=200)
    plt.close()
    ...
    with open(os.path.join(OUTPUT_DIR, 'training_stats_cv_baseline_official.json'), 'w') as f:
        json.dump(stats, f, indent=2)
    
    print(f"\n{'='*70}")
    print("🎉 Baseline EGNN Official Training Complete (Cv)!")
    print(f"{'='*70}")

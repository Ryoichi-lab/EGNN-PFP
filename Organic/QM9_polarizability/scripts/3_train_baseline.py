...
"""
QM9 B3LYP Polarizability (α) 予測 - EGNN Baseline
EGNN公式実装の忠実な再現（PFPなし）+ ターゲット標準化 + Early Stopping
...
def compute_mean_mad(loader, device):
    ...
    print("  Collecting polarizability target values...")
    for batch in loader:
    ...
    print(f"  Polarizability Target statistics:")
    print(f"    Mean: {mean:.4f} Bohr³")
    print(f"    MAD:  {mad:.4f} Bohr³")
    print(f"    Min:  {all_targets.min():.4f} Bohr³")
    print(f"    Max:  {all_targets.max():.4f} Bohr³")
...
def train_epoch_official(model, loader, optimizer, device, mean, mad):
    ...
    for batch in tqdm(loader, desc="Training", leave=False):
        ...
        mae = torch.abs(pred_original - target).mean()
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    ax1.set_xlabel('True Polarizability (Bohr³)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted Polarizability (Bohr³)', fontsize=12, fontweight='bold')
    r2_stat = r2_score(targets, predictions)
    ax1.set_title(f'EGNN Baseline - Epoch {epoch}\nMAE: {mae:.4f} Bohr³, R²: {r2_stat:.4f}', 
    ...
    ax2.set_xlabel('Prediction Error (Bohr³)', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Frequency', fontsize=12, fontweight='bold')
    ax2.set_title(f'Error Distribution\nMean: {errors.mean():.4f} Bohr³, Std: {errors.std():.4f} Bohr³', 
                  fontsize=14, fontweight='bold')
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'polarizability_baseline_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def plot_training_history(history, output_dir):
    ...
    ax1.axhline(y=0.085, color='red', linestyle='--', linewidth=2, label='Paper EGNN (0.085 Bohr³)')
    ax1.set_xlabel('Epoch', fontsize=12, fontweight='bold')
    ax1.set_ylabel('MAE (Bohr³)', fontsize=12, fontweight='bold')
    ax1.set_title('Training History (Baseline)', fontsize=14, fontweight='bold')
    ax1.legend(fontsize=10)
    ax1.grid(True, alpha=0.3)
    
    # Learning Rate推移
    ax2 = axes[1]
    ax2.plot(history['learning_rates'], linewidth=2, color='green')
...
def main():
    # 絶対パス設定
    QM9_POLAR_DIR = "/home/users/uchiyama/QM9_polarizability"
    
    # ★PFP版のグラフを使用（ベースラインは特徴抽出時にone-hotのみ使う）
    GRAPH_DIR = os.path.join(QM9_POLAR_DIR, "graphs_polarizability_qm9_B3LYP_baseline_perfect")
    OUTPUT_DIR = os.path.join(QM9_POLAR_DIR, "training_baseline_polarizability_perfect_0115")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP Polarizability (α) 予測 - EGNN Baseline")
    print("EGNN公式実装（PFPなし）+ カットオフ5Å + ターゲット標準化 + Early Stopping")
    ...
    print(f"\n📊 データ形状確認:")
    print(f"  元のノード特徴: {sample.x.shape}")
    print(f"  エッジ特徴: {sample.edge_attr.shape}")
    print(f"  座標: {sample.pos.shape}")
    print(f"  Polarizability: {sample.y.shape}")
    ...
    # Early Stopping設定
    patience = 500
    min_improvement = 0.001  # 0.001 Bohr³
    
    print(f"\n{'='*70}")
    print("⚙️  EGNN公式実装準拠設定（Baseline: PFPなし）")
    print(f"{'='*70}")
    print(f"  Target:        Polarizability (α)")
    ...
    print(f"  Early Stop:    Patience={patience}, Min improvement={min_improvement} Bohr³")
    print(f"{'='*70}")
    
    PAPER_MAE = 0.085  # Bohr³
    ...
    for epoch in range(num_epochs):
        ...
        print(f"  Train: Loss={train_loss:.4f}, MAE={train_mae:.4f} Bohr³")
        print(f"  Val:   MAE={val_mae:.4f} Bohr³  |  LR={current_lr:.6f}")
        
        if val_mae < best_val_mae - min_improvement:
            improvement = best_val_mae - val_mae if best_val_mae != float('inf') else 0
            ...
            if epoch == 0:
                print(f"  ⭐ Initial best: {val_mae:.4f} Bohr³")
            else:
                print(f"  ⭐ New best! Improved by {improvement:.4f} Bohr³ (Patience reset)")
        ...
        if patience_counter >= patience:
            ...
            print(f"    Best Val MAE: {best_val_mae:.4f} Bohr³ (Epoch {best_epoch})")
            break
    ...
    results = {
        ...
        'model_version': 'egnn_baseline_from_pfp_graph',
        'best_val_mae': float(best_val_mae),
        'best_epoch': best_epoch,
        'total_epochs_trained': epoch + 1,
        'early_stopped': patience_counter >= patience,
        'paper_egnn_mae': PAPER_MAE,
        'num_params': num_params,
        'config': {
            'input': '5-dim one-hot (H,C,N,O,F)',
            'edge_feat': 'None',
            'hidden_dim': 128,
            'num_layers': 7,
    ...
    print(f"Paper EGNN MAE: {PAPER_MAE} Bohr³")

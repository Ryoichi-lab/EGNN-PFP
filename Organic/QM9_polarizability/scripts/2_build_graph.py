...
# ===== 絶対パス設定 =====
QM9_polarizability_DIR = "/home/users/uchiyama/QM9_polarizability"
XYZ_DIR = os.path.join(QM9_polarizability_DIR, "qm9_xyz_polarizability") # XYZファイルがあるフォルダ
DESCRIPTOR_B3LYP_PATH = "/home/users/uchiyama/QM9_dipole/pfp_descriptors_qm9_B3LYP.npz"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(QM9_polarizability_DIR, "graphs_polarizability_qm9_B3LYP_baseline_perfect")
OUTPUT_DIR_PFP = os.path.join(QM9_polarizability_DIR, "graphs_polarizability_qm9_B3LYP_pfp_perfect")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("QM9データセット B3LYP/6-31G(2df,p) polarizability (Electronic spatial extent) 用グラフ作成")
print("EGNN論文準拠（Anderson et al. 2019 split: 100k/18k/13k）")
print("="*60)

# ===== XYZファイルから polarizability を読み取る関数 =====
def read_xyz_with_polarizability(xyz_path):
    """
    XYZファイルから原子座標とpolarizabilityを読み取る
    2行目に 'polarizability=...' が書かれている前提
    """
    try:
        ...
        # polarizability=... を抽出
        match = re.search(r'polarizability=([-\d\.Ee+]+)', comment_line)
        if match:
            polarizability_val = float(match.group(1))
        else:
        ...
        return atoms, np.array(positions), polarizability_val
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, cutoff=None):  # cutoffをNoneに
    try:
        atoms, positions, polarizability_val = read_xyz_with_polarizability(xyz_path)
        
        if atoms is None or positions is None or polarizability_val is None:
            return None
        ...
        return Data(
            ...
            y=torch.FloatTensor([polarizability_val]),
            mol_id=mol_id
        )
    except Exception as e:
...
train_graphs_baseline = create_molecular_graphs_baseline(train_mol_ids, XYZ_DIR, "polarizability_Baseline_Train")
val_graphs_baseline = create_molecular_graphs_baseline(val_mol_ids, XYZ_DIR, "polarizability_Baseline_Val")
test_graphs_baseline = create_molecular_graphs_baseline(test_mol_ids, XYZ_DIR, "polarizability_Baseline_Test")

print("\n【2/2】EGNN×PFP グラフ生成中...")
train_graphs_pfp = create_molecular_graphs_pfp(train_mol_ids, descriptor_data, XYZ_DIR, "polarizability_PFP_Train")
val_graphs_pfp = create_molecular_graphs_pfp(val_mol_ids, descriptor_data, XYZ_DIR, "polarizability_PFP_Val")
test_graphs_pfp = create_molecular_graphs_pfp(test_mol_ids, descriptor_data, XYZ_DIR, "polarizability_PFP_Test")

# ===== 統計情報収集 =====
all_polarizability_baseline = [g.y.item() for g in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline]
all_polarizability_pfp = [g.y.item() for g in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp]

all_polarizability_baseline = np.array(all_polarizability_baseline)
all_polarizability_pfp = np.array(all_polarizability_pfp)
...
stats_baseline = {
    ...
    'target': 'polarizability (Electronic spatial extent)',
    'unit': 'Bohr²',
    ...
    'random_seed': 42,
    'polarizability_statistics': {
        'min': float(all_polarizability_baseline.min()),
        'max': float(all_polarizability_baseline.max()),
        'mean': float(all_polarizability_baseline.mean()),
        'std': float(all_polarizability_baseline.std())
    }
}
...
stats_pfp = {
    ...
    'random_seed': 42,
    'polarizability_statistics': {
        'min': float(all_polarizability_pfp.min()),
        'max': float(all_polarizability_pfp.max()),
        'mean': float(all_polarizability_pfp.mean()),
        'std': float(all_polarizability_pfp.std())
    }
}
...
print("\n" + "="*60)
print("✅ QM9 B3LYP polarizability グラフ作成完了!")
print("="*60)
print(f"\n【ベースライン】")
print(f"  保存先: {OUTPUT_DIR_BASELINE}")
print(f"  polarizability範囲: {stats_baseline['polarizability_statistics']['min']:.4f} - {stats_baseline['polarizability_statistics']['max']:.4f} Bohr²")
print(f"\n【EGNN×PFP】")
print(f"  保存先: {OUTPUT_DIR_PFP}")
print(f"  polarizability範囲: {stats_pfp['polarizability_statistics']['min']:.4f} - {stats_pfp['polarizability_statistics']['max']:.4f} Bohr²")
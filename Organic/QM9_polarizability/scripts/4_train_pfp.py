...
"""
QM9 B3LYP Polarizability (α) 予測 - EGNN×PFP
EGNN公式実装の忠実な再現 + PFP記述子 + ターゲット標準化 + Early Stopping
単位: Bohr^3
"""
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'polarizability_pfp_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def main():
    ...
    GRAPH_DIR = os.path.join(QM9_POLAR_DIR, "graphs_polarizability_qm9_B3LYP_pfp_perfect")
    OUTPUT_DIR = os.path.join(QM9_POLAR_DIR, "training_pfp_polarizability_egnn_official_128_attention_perfect_0110")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP Polarizability (α) 予測 - EGNN×PFP")
    print("EGNN公式実装の忠実な再現 + PFP記述子 + ターゲット標準化 + Early Stopping")
    print("単位: Bohr³")
    print("="*70)
    ...
    print(f"\n{'='*70}")
    print("⚙️  EGNN公式実装準拠設定 + PFP記述子 + ターゲット標準化")
    print(f"{'='*70}")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae - min_improvement:
            ...
            torch.save(checkpoint, os.path.join(OUTPUT_DIR, 'best_pfp_official_model.pth'))
    ...
    results = {
        ...
        'model_version': 'egnn_official_with_pfp',

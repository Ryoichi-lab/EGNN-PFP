...
"""
QM9データセットからHOMOエネルギー (homo, eV) と
...
output_dir = "/home/users/uchiyama/QM9_HOMO_LUMO/qm9_xyz_homo"
os.makedirs(output_dir, exist_ok=True)

# ==============================================================  
# 3. 物性インデックス（HOMO 'homo'）を特定
# ==============================================================  
...
try:
    homo_index = dataset.target.index('homo')
except AttributeError:
    # PyTorch GeometricのQM9では固定順序: index=2 が homo
    homo_index = 2
...
for i, data in enumerate(dataset):
    # --- homoを安全に取得（19要素対応） ---
    if data.y.ndim == 2:
        homo = float(data.y[0, homo_index])
    else:
        homo = float(data.y[homo_index])

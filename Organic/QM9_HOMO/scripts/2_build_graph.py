...
# ===== 絶対パス設定 =====
QM9_HOMO_DIR = "/home/users/uchiyama/QM9_HOMO"
XYZ_DIR = os.path.join(QM9_HOMO_DIR, "qm9_xyz_homo") # XYZファイルがあるフォルダ
DESCRIPTOR_B3LYP_PATH = "/home/users/uchiyama/QM9_dipole/pfp_descriptors_qm9_B3LYP.npz"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(QM9_HOMO_DIR, "graphs_HOMO_qm9_B3LYP_baseline_perfect")
OUTPUT_DIR_PFP = os.path.join(QM9_HOMO_DIR, "graphs_HOMO_qm9_B3LYP_pfp_perfect")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("QM9データセット B3LYP/6-31G(2df,p) HOMO (Electronic spatial extent) 用グラフ作成")
print("EGNN論文準拠（Anderson et al. 2019 split: 100k/18k/13k）")
print("="*60)

# ===== XYZファイルから HOMO を読み取る関数 =====
def read_xyz_with_HOMO(xyz_path):
    """
    XYZファイルから原子座標とHOMOを読み取る
    2行目に 'HOMO=...' が書かれている前提
    """
    try:
        ...
        # HOMO=... を抽出
        match = re.search(r'homo=([-\d\.Ee+]+)', comment_line)
        if match:
            HOMO_val = float(match.group(1))
        else:
        ...
        return atoms, np.array(positions), HOMO_val
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, cutoff=None):  # cutoffをNoneに
    try:
        atoms, positions, HOMO_val = read_xyz_with_HOMO(xyz_path)
        
        if atoms is None or positions is None or HOMO_val is None:
            return None
...
train_graphs_baseline = create_molecular_graphs_baseline(train_mol_ids, XYZ_DIR, "HOMO_Baseline_Train")
val_graphs_baseline = create_molecular_graphs_baseline(val_mol_ids, XYZ_DIR, "HOMO_Baseline_Val")
test_graphs_baseline = create_molecular_graphs_baseline(test_mol_ids, XYZ_DIR, "HOMO_Baseline_Test")

print("\n【2/2】EGNN×PFP グラフ生成中...")
train_graphs_pfp = create_molecular_graphs_pfp(train_mol_ids, descriptor_data, XYZ_DIR, "HOMO_PFP_Train")
val_graphs_pfp = create_molecular_graphs_pfp(val_mol_ids, descriptor_data, XYZ_DIR, "HOMO_PFP_Val")
test_graphs_pfp = create_molecular_graphs_pfp(test_mol_ids, descriptor_data, XYZ_DIR, "HOMO_PFP_Test")

# ===== 統計情報収集 =====
all_HOMO_baseline = [g.y.item() for g in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline]
all_HOMO_pfp = [g.y.item() for g in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp]
...
stats_baseline = {
    ...
    'target': 'HOMO (Electronic spatial extent)',
    'unit': 'Bohr²',
...
print("\n" + "="*60)
print("✅ QM9 B3LYP HOMO グラフ作成完了!")
print("="*60)
print(f"\n【ベースライン】")
print(f"  保存先: {OUTPUT_DIR_BASELINE}")
print(f"  HOMO範囲: {stats_baseline['HOMO_statistics']['min']:.4f} - {stats_baseline['HOMO_statistics']['max']:.4f} Bohr²")
print(f"\n【EGNN×PFP】")
print(f"  保存先: {OUTPUT_DIR_PFP}")
print(f"  HOMO範囲: {stats_pfp['HOMO_statistics']['min']:.4f} - {stats_pfp['HOMO_statistics']['max']:.4f} Bohr²")
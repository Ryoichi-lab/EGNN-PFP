...
"""
QM9 B3LYP R2 (Electronic Spatial Extent) 予測 - EGNN Baseline
...
def compute_mean_mad(loader, device):
    ...
    print("  Collecting R2 target values...")
    for batch in loader:
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    ax1.set_xlabel('True R2 (Bohr²)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted R2 (Bohr²)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'r2_baseline_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def plot_training_history(history, output_dir):
    ...
    ax1.axhline(y=0.106, color='red', linestyle='--', linewidth=2, label='Paper EGNN (0.106 Bohr²)')
...
def main():
    # 絶対パス設定
    QM9_R2_DIR = "/home/users/uchiyama/QM9_R2"
    
    # ★PFP版のグラフを使用（ベースラインは特徴抽出時にone-hotのみ使う）
    GRAPH_DIR = os.path.join(QM9_R2_DIR, "graphs_r2_qm9_B3LYP_baseline_perfect")
    OUTPUT_DIR = os.path.join(QM9_R2_DIR, "training_baseline_r2_from_pfp_0115")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP R2 (Electronic Spatial Extent) 予測 - EGNN Baseline")
    ...
    print(f"{'='*70}")
    print(f"  Target:        R2 (Electronic Spatial Extent)")
    ...
    results = {
        ...
        'config': {
            'input': '5-dim one-hot (H,C,N,O,F) from PFP graph',
            'edge_feat': 'None',
            'hidden_dim': 128,
            'num_layers': 7,

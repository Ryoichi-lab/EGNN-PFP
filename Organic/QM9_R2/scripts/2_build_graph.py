...
XYZ_DIR = os.path.join(QM9_R2_DIR, "qm9_xyz_R2") # XYZファイルがあるフォルダ
DESCRIPTOR_B3LYP_PATH = "/home/users/uchiyama/QM9_dipole/pfp_descriptors_qm9_B3LYP.npz"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(QM9_R2_DIR, "graphs_r2_qm9_B3LYP_baseline_perfect")
OUTPUT_DIR_PFP = os.path.join(QM9_R2_DIR, "graphs_r2_qm9_B3LYP_pfp_perfect")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("QM9データセット B3LYP/6-31G(2df,p) R2 (Electronic spatial extent) 用グラフ作成")
print("EGNN論文準拠（Anderson et al. 2019 split: 100k/18k/13k）")
print("="*60)

# ===== XYZファイルから R2 を読み取る関数 =====
def read_xyz_with_r2(xyz_path):
    """
    XYZファイルから原子座標とR2を読み取る
    2行目に 'R2=...' が書かれている前提
    """
    try:
        ...
        # R2=... を抽出
        match = re.search(r'R2=([-\d\.Ee+]+)', comment_line)
        if match:
        ...
        return atoms, np.array(positions), r2_val
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, cutoff=None):  # cutoffをNoneに
    try:
        atoms, positions, r2_val = read_xyz_with_r2(xyz_path)
        
        if atoms is None or positions is None or r2_val is None:
            return None
...
train_graphs_baseline = create_molecular_graphs_baseline(train_mol_ids, XYZ_DIR, "R2_Baseline_Train")
val_graphs_baseline = create_molecular_graphs_baseline(val_mol_ids, XYZ_DIR, "R2_Baseline_Val")
test_graphs_baseline = create_molecular_graphs_baseline(test_mol_ids, XYZ_DIR, "R2_Baseline_Test")

print("\n【2/2】EGNN×PFP グラフ生成中...")
train_graphs_pfp = create_molecular_graphs_pfp(train_mol_ids, descriptor_data, XYZ_DIR, "R2_PFP_Train")
val_graphs_pfp = create_molecular_graphs_pfp(val_mol_ids, descriptor_data, XYZ_DIR, "R2_PFP_Val")
test_graphs_pfp = create_molecular_graphs_pfp(test_mol_ids, descriptor_data, XYZ_DIR, "R2_PFP_Test")

# ===== 統計情報収集 =====
all_r2_baseline = [g.y.item() for g in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline]
all_r2_pfp = [g.y.item() for g in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp]

all_r2_baseline = np.array(all_r2_baseline)
all_r2_pfp = np.array(all_r2_pfp)
...
stats_baseline = {
    ...
    'target': 'R2 (Electronic spatial extent)',
    'unit': 'Bohr²',
    ...
    'random_seed': 42,
    'R2_statistics': {
        'min': float(all_r2_baseline.min()),
        'max': float(all_r2_baseline.max()),
        'mean': float(all_r2_baseline.mean()),
        'std': float(all_r2_baseline.std())
    }
}
...
stats_pfp = {
    ...
    'random_seed': 42,
    'R2_statistics': {
        'min': float(all_r2_pfp.min()),
        'max': float(all_r2_pfp.max()),
        'mean': float(all_r2_pfp.mean()),
        'std': float(all_r2_pfp.std())
    }
}
...
print("\n" + "="*60)
print("✅ QM9 B3LYP R2 グラフ作成完了!")
print("="*60)
print(f"\n【ベースライン】")
print(f"  保存先: {OUTPUT_DIR_BASELINE}")
print(f"  R2範囲: {stats_baseline['R2_statistics']['min']:.4f} - {stats_baseline['R2_statistics']['max']:.4f} Bohr²")
print(f"\n【EGNN×PFP】")
print(f"  保存先: {OUTPUT_DIR_PFP}")
print(f"  R2範囲: {stats_pfp['R2_statistics']['min']:.4f} - {stats_pfp['R2_statistics']['max']:.4f} Bohr²")
...
"""
QM9 B3LYP ZPVE (Zero-point vibrational energy) 予測 - EGNN×PFP
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'zpve_pfp_official_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def plot_training_history(history, output_dir):
    ...
    ax1.axhline(y=1.5, color='red', linestyle='--', linewidth=2, label='Paper EGNN (1.5 meV)')
...
def main():
    ...
    GRAPH_DIR = os.path.join(QM9_ZPVE_DIR, "graphs_ZPVE_qm9_B3LYP_pfp_perfect")
    OUTPUT_DIR = os.path.join(QM9_ZPVE_DIR, "training_pfp_zpve_egnn_official_128_attention_perfect_0112")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP ZPVE (Zero-point vibrational energy) 予測 - EGNN×PFP")

...
# ===== 絶対パス設定 =====
QM9_U_DIR = "/home/users/uchiyama/QM9_ZPVE"
XYZ_DIR = os.path.join(QM9_U_DIR, "qm9_xyz_zpve") # XYZファイルがあるフォルダ
DESCRIPTOR_B3LYP_PATH = "/home/users/uchiyama/QM9_dipole/pfp_descriptors_qm9_B3LYP.npz"

# 出力ディレクトリ（2種類）
OUTPUT_DIR_BASELINE = os.path.join(QM9_U_DIR, "graphs_ZPVE_qm9_B3LYP_baseline_perfect")
OUTPUT_DIR_PFP = os.path.join(QM9_U_DIR, "graphs_ZPVE_qm9_B3LYP_pfp_perfect")

os.makedirs(OUTPUT_DIR_BASELINE, exist_ok=True)
os.makedirs(OUTPUT_DIR_PFP, exist_ok=True)

print("="*60)
print("QM9データセット B3LYP/6-31G(2df,p) U (Electronic spatial extent) 用グラフ作成")
print("EGNN論文準拠（Anderson et al. 2019 split: 100k/18k/13k）")
print("="*60)

# ===== XYZファイルから U を読み取る関数 =====
def read_xyz_with_U(xyz_path):
    """
    XYZファイルから原子座標とUを読み取る
    2行目に 'U=...' が書かれている前提
    """
    try:
        ...
        # U=... を抽出
        match = re.search(r'zpve=([-\d\.Ee+]+)', comment_line)
        if match:
        ...
        return atoms, np.array(positions), U_val
...
def create_graph_from_xyz_baseline(xyz_path, mol_id, cutoff=None):  # cutoffをNoneに
    try:
        atoms, positions, U_val = read_xyz_with_U(xyz_path)
        
        if atoms is None or positions is None or U_val is None:
            return None
...
train_graphs_baseline = create_molecular_graphs_baseline(train_mol_ids, XYZ_DIR, "U_Baseline_Train")
val_graphs_baseline = create_molecular_graphs_baseline(val_mol_ids, XYZ_DIR, "U_Baseline_Val")
test_graphs_baseline = create_molecular_graphs_baseline(test_mol_ids, XYZ_DIR, "U_Baseline_Test")

print("\n【2/2】EGNN×PFP グラフ生成中...")
train_graphs_pfp = create_molecular_graphs_pfp(train_mol_ids, descriptor_data, XYZ_DIR, "U_PFP_Train")
val_graphs_pfp = create_molecular_graphs_pfp(val_mol_ids, descriptor_data, XYZ_DIR, "U_PFP_Val")
test_graphs_pfp = create_molecular_graphs_pfp(test_mol_ids, descriptor_data, XYZ_DIR, "U_PFP_Test")

# ===== 統計情報収集 =====
all_U_baseline = [g.y.item() for g in train_graphs_baseline + val_graphs_baseline + test_graphs_baseline]
all_U_pfp = [g.y.item() for g in train_graphs_pfp + val_graphs_pfp + test_graphs_pfp]

all_U_baseline = np.array(all_U_baseline)
all_U_pfp = np.array(all_U_pfp)
...
stats_baseline = {
    ...
    'target': 'U (Electronic spatial extent)',
    'unit': 'Bohr²',
    ...
    'random_seed': 42,
    'U_statistics': {
        'min': float(all_U_baseline.min()),
        'max': float(all_U_baseline.max()),
        'mean': float(all_U_baseline.mean()),
        'std': float(all_U_baseline.std())
    }
}
...
stats_pfp = {
    ...
    'random_seed': 42,
    'U_statistics': {
        'min': float(all_U_pfp.min()),
        'max': float(all_U_pfp.max()),
        'mean': float(all_U_pfp.mean()),
        'std': float(all_U_pfp.std())
    }
}
...
print("\n" + "="*60)
print("✅ QM9 B3LYP U グラフ作成完了!")
print("="*60)
print(f"\n【ベースライン】")
print(f"  保存先: {OUTPUT_DIR_BASELINE}")
print(f"  U範囲: {stats_baseline['U_statistics']['min']:.4f} - {stats_baseline['U_statistics']['max']:.4f} Bohr²")
print(f"\n【EGNN×PFP】")
print(f"  保存先: {OUTPUT_DIR_PFP}")
print(f"  U範囲: {stats_pfp['U_statistics']['min']:.4f} - {stats_pfp['U_statistics']['max']:.4f} Bohr²")
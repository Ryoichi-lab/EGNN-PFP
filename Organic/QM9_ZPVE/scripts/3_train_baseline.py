...
"""
QM9 B3LYP ZPVE (Zero-point vibrational energy) 予測 - EGNN Baseline
...
def compute_mean_mad(loader, device):
    ...
    print("  Collecting ZPVE target values...")
    for batch in loader:
    ...
    print(f"  ZPVE Target statistics:")
...
def plot_predictions(predictions, targets, epoch, mae, output_dir):
    ...
    ax1.set_xlabel('True ZPVE (meV)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Predicted ZPVE (meV)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'zpve_baseline_predictions_epoch_{epoch}.png'), dpi=200)
    plt.close()
...
def plot_training_history(history, output_dir):
    ...
    ax1.axhline(y=1.55, color='red', linestyle='--', linewidth=2, label='Paper EGNN (1.55 meV)')
...
def main():
    ...
    GRAPH_DIR = os.path.join(QM9_ZPVE_DIR, "graphs_ZPVE_qm9_B3LYP_baseline_perfect")
    OUTPUT_DIR = os.path.join(QM9_ZPVE_DIR, "training_baseline_zpve_from_pfp_0115")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("="*70)
    print("QM9 B3LYP ZPVE (Zero-point vibrational energy) 予測 - EGNN Baseline")
    ...
    # Early Stopping設定
    patience = 700
    min_improvement = 0.1  # 0.1 meV
    
    print(f"\n{'='*70}")
    print("⚙️  EGNN公式実装準拠設定（Baseline: PFPなし）")
    print(f"{'='*70}")
    print(f"  Target:        ZPVE (Zero-point vibrational energy)")
    ...
    print(f"  Early Stop:    Patience={patience}, Min improvement={min_improvement} meV")
    print(f"{'='*70}")
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_val_mae - min_improvement:
            ...
            if epoch == 0:
                ...
                print(f"  ⭐ New best! Improved by {improvement:.2f} meV (Patience reset)")
    ...
    print(f"Paper EGNN MAE: {PAPER_MAE} meV")
